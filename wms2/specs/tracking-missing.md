...
- **[WF-NEW-E] — FIXED 2026-08-03 (this page's half).** The `#inbox1` pane headers now ship the four-page-majority strings `Comments mentioning me · Click to open the order` / `Saved comments · Click to open the order`; `Mark all read` and `Unstar to remove from the list` were already canonical and are unchanged. QA-CMT-03 / QA-CMT-05 are re-baselined to the canonical strings and §3.6's `[WF]`-vs-`[ADMIN]` table is collapsed. The corpus-wide half — publishing the hub strings in `[G-7]` — **also landed 2026-08-03**: `[G-7]` v1.2 publishes HUB-1…HUB-7 and all eight wireframes now render them, so cross-page defect m3a D7 is closed. **Also filed in `_wireframe-fixes.md` §B.**
...
|---|---|---|---|---|
| 1 | `Tracking No` | Auto-collected by the View Orders scanner at scan time; carried on the intake payload | `.num` tabular figures, digits only, no grouping (e.g. `10323100841207`). Nullability is settled by `[PD-66 · OWNER-DECIDED 2026-08-03]`: a pool entry **always carries an identifier** — either a tracking number or an order number — and View Orders M2b rejects a registration carrying neither, so the pool never holds an item with no identifier at all (§9.1, `[E-3]`). **Namespace:** the number is stored with the namespace the scan originated in — `outbound` for a customer parcel, `inbound` for an unrequested supplier arrival `[BR-11]` — and the uniqueness guard `[BR-19]`, `[E-11]` is evaluated **within that namespace only**. A pool item whose number is `inbound` therefore does **not** collide with an identical `outbound` number, which is exactly the configuration `[E-33]` describes (an unrequested-inbound parcel matched to a customer order): the namespace check is silent there by design, so `[E-33]`'s only defence remains the memo surfaced in `[L-M1]` *(spec-authored)* | Read-only |
...
|---|---|
| Mentions pane header (`[G-7]` HUB-1) | `Comments mentioning me · Click to open the order` |
...
QA-CMT-03 / QA-CMT-05 (`[WF]`) are re-baselined to these strings and QA-CMT-15 (`[ADMIN]`) is unchanged. The corpus-wide half of `WF-NEW-E` — publishing the hub strings in `[G-7]` as byte-exact contract so the other divergent pages converge — **landed 2026-08-03**: the four strings above are `[G-7]` HUB-1 / HUB-2 / HUB-3 / HUB-4, and `[G-7]` is now the source; this table quotes it rather than owning it. Cross-page defect m3a D7 is closed. This page builds no search pane, so HUB-5 / HUB-6 / HUB-7 do not apply here. `Mark all read` is right-aligned and blue.
...
| `Tracking {n}` | pool item tracking no. | `[PD-66 · OWNER-DECIDED 2026-08-03]` — an entry always carries an identifier, so this fragment is absent only when the identifier the row carries is the order number; the summary then shows `Order no. {n}` in the following slot `[E-2]`, `[E-3]`, §9.1 |
...
IDs are page-scoped and stable. `[E-1]`…`[E-31]` preserve the Lens-B plan numbering; `[E-32]`…`[E-46]` were added during drafting; `[E-47]`…`[E-72]` were added during the audit pass. **72 cases, no numbering gaps.** Every case states the expected behavior, `[E-3]` included: the NO-DEFAULT gate that once held it unspecified was lifted when the owner decided `[PD-66]` on 2026-08-03, so **no case is left as "TBD"** (§9.1).
...
| **E-3** | The tracking number could not be auto-collected (label destroyed) | **Decided — the identifier-less case does not exist** `[PD-66 · OWNER-DECIDED 2026-08-03]`: either a tracking number **or** an order number is always present, so the registration contract stands unchanged — an identifier is required and View Orders M2b rejects a registration carrying neither. The `[DC-17]` rescan-resolves loop the whole flow depends on is therefore preserved. Where the tracking number is the missing one, the identifier the row carries is the order number the registrant supplied in the M2 lookup, rendered in column 2 `[E-2]`, `[BR-12]`. Asserted by QA-VAL-10. *(History: this path was NO-DEFAULT and carried no acceptance criteria until the owner ruled on 2026-08-03 — see §9.1.)* |
...
- **BLOCKED** — a scenario that cannot have a verdict until an owner decision lands. Report as BLOCKED, never as PASS or FAIL. **As of 2026-08-03 no scenario in this document carries this marker:** QA-VAL-10 was the only one and it was un-blocked when the owner decided `[PD-66]` (§9.1). The definition stays here so a future NO-DEFAULT question has a defined reporting mode.
...
3. **`.mockwrap` matches two elements and they nest.** At a narrowed viewport **both** overflow (outer 1280→viewport, inner 1457→1248) and **both** must be scrolled to bring the `Action` column into view; scrolling only the outer one leaves `.xdel` off-screen. Applies to QA-A11Y-05. QA-EMPTY-01 depends on the same distinction from the other side: its collapse guard measures the **outer** wrapper (the one wrapping `.mock`, whose height is held by `.mock`'s `min-height:520px`), because the inner wrapper contains only the table and legitimately shrinks to the header row once every row is removed.
...
**Totals: 168 scenarios — 66 [WF], 102 [ADMIN]. Negative scenarios: 61 (36.3%), above the 25% floor.** **`BLOCKED`: 0.** QA-VAL-10 was the single `BLOCKED` scenario until the owner decided `[PD-66]` on 2026-08-03; it now carries assertable criteria and yields a normal verdict. It stays counted inside the 168 and inside the 102 `[ADMIN]`, and it was not added, retired, re-tiered, or re-tagged, so every total and per-block count below is unchanged by the un-blocking. *(Re-counted after the 2026-08-03 re-baselining: the WF-6/WF-NEW-D/WF-NEW-E fixes rewrote scenario bodies — QA-XDEL-01…03, QA-WFQ-02/03/07, QA-CMT-03/05, QA-LOAD-07/09/11/12, QA-EMPTY-01, QA-VAL-06, QA-XDEL-04/06, QA-NEG-03's note, QA-DATA-09 — but no scenario was added, retired, re-tiered, or re-tagged, so every total and per-block count below is unchanged.)*
...
**QA-CMT-15 [ADMIN]** — Given the admin's Comments hub is open, Then the Mentions pane header reads exactly `Comments mentioning me · Click to open the order` And the Saved pane header reads exactly `Saved comments · Click to open the order` — the corpus-canonical strings, §3.6 — And the unstar hint reads `Unstar to remove from the list` And the read-all action reads `Mark all read` — `[G-7]` HUB-1 / HUB-2 / HUB-3 / HUB-4. *(Since the 2026-08-03 `WF-NEW-E` fix the `[WF]` counterparts QA-CMT-03 / QA-CMT-05 assert the same strings — the two tiers no longer diverge on this page.)*
...
**QA-VAL-10 [ADMIN]** `[E-3]` `[PD-66 · OWNER-DECIDED 2026-08-03]` *(formerly BLOCKED — un-blocked when the owner ruled on 2026-08-03)* — The owner decided the identifier-less case does not exist: either a tracking number or an order number is always present, and the identifier-required registration contract stands. Given a View Orders M2b registration attempt carrying **neither** a tracking number nor an order number, Then the registration is refused with a validation error, **no** pool row is created on this page, and no intake event persists (`[DC-1]`, `[DC-5]` absent); And given a registration carrying at least one of the two identifiers, Then it is not refused on identifier grounds — the row reaches the pool as usual `[E-2]`, `[BR-12]` (M2b's other validations, the mandatory product pick and the qty rule, are owned by the **view-orders** spec and still apply). Cross-page counterpart: view-orders QA-CV-22 (§9.1).
...
**QA-EMPTY-01 [WF]** — Given all three rows have been removed via `✕` → `[L-M2]` confirm (reason `No action needed`, one confirm each), Then `#poolCount` and `#poolCountBottom` both read `0`, And `.poolhead` is still rendered with a bounding-box height > 0, And the layout has not collapsed, measured as: the **outer** `.mockwrap`'s bounding-box height is **> 300 px** — the outer wrapper is the one wrapping `.mock` (§8.0 rule 3), whose height is held at ≥ 520 px by `.mock`'s `min-height`; the **inner** `.mockwrap` wraps only the table and legitimately shrinks to the header row's height once every row is removed, so it is **not** the collapse signal and must not be the measured element — the pool table's `<thead>` is still rendered with its 12 `<th>`, and `#poolCountBottom`'s bounding box lies **below** the table's `<thead>` `[E-19]`.
...
| E-3 | QA-VAL-10 (*was BLOCKED; un-blocked when `[PD-66]` was owner-decided 2026-08-03, §9.1*) | E-39 | QA-VAL-08 |
...
**72 cases; 71 asserted; 1 declared exception (`[E-4]`).** `[E-3]` now carries a behavioral assertion: QA-VAL-10 covered only the *decision gap* (as `BLOCKED`, with no verdict) until the owner decided `[PD-66]` on 2026-08-03, and since then it asserts the identifier-required registration contract and yields a normal verdict. The 71/1 split is unchanged — `[E-3]` was already inside the 71.

---

## 9. Out of Scope & Open Questions

Per `_review.md` §3.8 this section carries **only** NO-DEFAULT owner questions, developer-time decisions, and out-of-scope pointers. Owner questions that already have a provisional default live in `_provisional-decisions.md` and are tagged inline above — they are not repeated here. This page's single NO-DEFAULT question (`[PD-66]`) was **owner-decided on 2026-08-03**; §9.1 is kept as the resolution record, not as an open item.

### 9.1 NO-DEFAULT open question — **RESOLVED 2026-08-03** (owner-decided; behavior is now specified)

**`[PD-66]` — Can an item enter the pool with NO tracking number (label destroyed, auto-collection failed)?**
- **Status: RESOLVED — OWNER-DECIDED 2026-08-03.** **Answer: the case does not exist** — either a tracking number or an order number is always present. The registration contract stands unchanged: an identifier is required, and View Orders M2b never accepts a pool entry that carries neither.
- **Why it was blocking (kept for provenance):** if an identifier-less item were allowed, a "match" would have nothing to write onto the product line, which would break the `[DC-17]` rescan-resolves loop that the entire flow depends on `[BR-5]`. The decision preserves that loop; it also settles the registration contract on **View Orders M2b**, not just this page.
- **Affected units — all now settled:** `[L-1]` column 1 (nullability — §3.2.2), `[L-M1]` summary card (`Tracking {n}` fragment — §3.7), `[E-3]`, §3.4.3 step 2.
- **QA consequence:** **QA-VAL-10 is no longer `BLOCKED`** — it asserts the identifier-required contract and yields a normal verdict (§8.10). No scenario in this document is `BLOCKED`; §8 totals are unchanged.
- **Owner:** SkinSeoul Director (Yongwon). **Decided:** 2026-08-03, recorded in `_provisional-decisions.md` under `[PD-66] · OWNER-DECIDED`. **Cross-page counterpart:** view-orders §9.2 OQ-1 (RESOLVED), `[E-63]`, QA-CV-22.
- **History (do not re-open):** until 2026-08-03 this was the page's one NO-DEFAULT question — no default was adopted and no behavior was specified anywhere in this document; QA-VAL-10 was reported as `BLOCKED` and an implementation was required to escalate rather than pick a behavior.

**Provisional decisions carried by this page** (behavior IS specified; listed for the owner's review pass, not re-opened here): PD-1, PD-3, PD-4, PD-6, PD-7, PD-8, PD-16, PD-61, PD-62, PD-63, PD-64, PD-65, PD-67, PD-80 — **14 entries still OWNER-PENDING**. **Owner-CONFIRMED 2026-08-03: PD-5 and PD-60** (removal confirm + reason + toast; reflected in `[L-M2]`, `[BR-9]`, `[DC-14]`, `[DC-29]`, and the wireframe's M2 modal — no longer reversible-by-tag, so their inline tags read `· CONFIRMED 2026-08-03`). Cross-referenced only: PD-2 (cannot land — no outbound button), PD-22 (Order Detail), **PD-51 (Order Management — the sample-definition source; NO-DEFAULT, cited in §9.3)**, PD-68 (Closing), PD-85 (Inbound Request) — **5 entries**. Total distinct PDs cited by this document: **21 carried or cross-referenced** (14 pending + 2 confirmed + 5 cross-referenced), plus `[PD-66]` above, which is neither carried nor cross-referenced but **owned** by this page as its one NO-DEFAULT question — owner-decided 2026-08-03 and therefore no longer open (§9.1) — **22 in all**.
...
| 2026-08-03 | `[PD-66]` left **NO-DEFAULT**: whether an item may enter the pool with no tracking number is undecided and blocks a View Orders validation rule — **superseded the same day by the owner decision; see the `[PD-66]` OWNER-DECIDED row below** | PD register | §9.1, `[E-3]`, QA-VAL-10 (BLOCKED) |
...
| 2026-08-03 | **`WF-NEW-E` filed and fixed — Comments-hub pane headers diverged from the majority strings.** The wireframe adopted `Comments mentioning me · Click to open the order` / `Saved comments · Click to open the order`; `Mark all read` and `Unstar to remove from the list` were already canonical. The corpus-wide half landed the same day: **`[G-7]` v1.2 publishes HUB-1…HUB-7** as byte-exact contract and all eight wireframes converged, closing m3a D7 | cross-page verification m3a D7; owner-approved batch | §2.5, §3.6, QA-CMT-03, QA-CMT-05, QA-CMT-15 |
...
| 2026-08-03 | **`[PD-66]` OWNER-DECIDED — the identifier-less case does not exist.** Either a tracking number **or** an order number is always present, so the identifier-required registration contract on View Orders M2b stands unchanged and the `[DC-17]` rescan-resolves loop is preserved. Supersedes the NO-DEFAULT row above: `[E-3]` now states behavior, `[L-1]` column 1 and `[L-M1]`'s `Tracking {n}` fragment are settled, **QA-VAL-10 is un-blocked** (no scenario in this document is `BLOCKED`), and §9.1 reads RESOLVED with the question kept as history. §8 totals unchanged — nothing added, retired, re-tiered, or re-tagged. The repo text had lagged the decision; view-orders had already reflected it (§3.12, `[E-63]`, QA-CV-22, §9.2 OQ-1) | owner decision 2026-08-03; PD register `[PD-66]`; pre-handoff ledger F-20 / F-25 | §3.2.2 col. 1, §3.7, §7 preamble, `[E-3]`, §8.0, QA-VAL-10, §8.18, §9.1 |
| 2026-08-03 | **QA-EMPTY-01 re-anchored to the outer `.mockwrap`.** Its collapse guard measured the **inner** `.mockwrap`, which wraps only the table and drops to the header row's height (~42 px) once all three rows are removed — the `> 300 px` clause was unsatisfiable by construction and produced a false FAIL. The measurement now targets the outer wrapper, whose height is held at ≥ 520 px by `.mock`'s `min-height:520px` (the m2 run recorded `mockH: 520` in the emptied state). §8.0 rule 3 gained the inner/outer note. Tier, tag and all counts unchanged | pre-handoff QA re-run (`_verify/prehandoff/`), ledger F-6; m2 journal measurement | §8.0 rule 3, §8.11 QA-EMPTY-01 |
