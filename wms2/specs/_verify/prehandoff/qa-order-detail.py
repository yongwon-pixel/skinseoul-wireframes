...
def qa_map_2(page):
    ...
    eq(T(page, ".wf-bar .hint"),
       "v1 — Based on the live admin (Order #400001) · Purple numbers = new/changed annotations",
       ".wf-bar .hint")
...
def qa_hub_6(page):
    ...
    chk(len(st2["items"]) == 2 and "Order 400001" in st2["items"][0]
        and "Order 407301" in st2["items"][1],
        "items in order: 400001 first, 407301 second", st2["items"])
...
def qa_hub_8(page):
    ...
    chk(len(st["items"]) == 3
        and "Order 400001" in st["items"][0]
...
def qa_edit_4(page):
    ...
    eq(st["vals"], ["0", "INBOUNDED", "JIT (Coupang)", "12101316464794", "2026-06-30",
                    "₩17,100", "10323100000001", "coupang…7923"],
...
def qa_ren_9(page):
    ...
    eq(st["row"], ["YT2618100700000001", "34YEM000000000000000001", "DELIVERED",
...
def qa_ren_10(page):
    ...
    eq(st["li_tn"], "10323100000001", "line-item Tracking Number (SKU 100043697)")
    eq(st["ft_tn"], "34YEM000000000000000001", "Fulfillment Tracking Tracking Number")
...
def qa_sub_2(page):
    ...
    eq(st["h2"], "Order # 400001", ".otitle h2")
...
def qa_sub_5(page):
    ...
    eq(st["bill"]["Email"], "jane.doe@example.com", "Billing Email")
    eq(st["bill"]["Phone"], "+61400000001", "Billing Phone")
