...
def qa_scan_01(page):
    ...
    if info:
        ...
        s.eq("value", "YT2618100700000004", info["v"])
...
def qa_verdict_02(page):
    ...
    s.has("okline contains order line", "Order 413540 · Tracking YT2618100700000004 · Prepare Shipment",
          t(page, "#s1 .okline"))
...
def qa_dup_03(page):
    ...
    if s.ok("row #2 exists in #s1", bool(cells), "exists", cells):
        s.eq("tracking", "YT2618100700000005", cells[2])
...
def qa_m1_02(page):
    ...
    s.has("body contains the line", "Tracking YT2618100700000006 · 1 item · Status Processing",
...
def qa_del_01(page):
    ...
    s.eq("#scandelInfo reads exactly", "#3 · YT2618100700000005", t(page, "#scandelInfo"))
    return s.finish()
...
def qa_del_10(page):
    ...
    s.ok("#m-scandel gains class open", has_class(page, "#m-scandel", "open"), "open", "")
    s.eq("#scandelInfo retains its default text", "#7 · YT2618100700000007", t(page, "#scandelInfo"))
    return s.finish()
...
def qa_chrome_03(page):
    ...
    load(page); state(page, "s1")
    s.eq("#s1 .toast span reads", "✓ Outbound confirmed — YT2618100700000004",
...
def qa_chrome_06(page):
    ...
    full = ev(page, "() => [...document.querySelectorAll('#s1 table.tbl td')]"
                   ".some(td => window.__t(td) === 'YT2618100700000005')")
