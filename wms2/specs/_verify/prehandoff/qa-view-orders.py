#!/usr/bin/env python3
"""
Pre-handoff QA re-execution of view-orders.md §8 [WF] scenarios (all 136).

Grading follows the CURRENT spec's §8.0 execution protocol literally:
  rule 6  - text asserts on textContent with whitespace runs collapsed+trimmed;
            byte-for-byte raw comparison is explicitly NOT a valid reading.
  rule 6b - strip annotation chrome (.dot descendants; a modal header's trailing
            button.x) before every text comparison and when locating controls by
            text. Verdicts are therefore graded on the rule-6b-normalized value.
  rule 7  - colours via getComputedStyle; transparent == 'rgba(0, 0, 0, 0)'.
  rule 7b - visible == el.getClientRects().length > 0.
  rule 7c - modal footer = .foot; hub panes = .tabpane[data-pane=...].
...
// §8.0 rule 6b: annotation-stripped comparison is the normative reading -> grade on `cleaned`.
window.__mk2 = (lit, cleaned, ev, extra) => Object.assign({v: cleaned ? 'PASS' : 'FAIL', ev: ev, literalAlsoHeld: !!lit}, extra||{});
window.__qa.nq = s => (s==null?'':String(s)).replace(/[“”]/g,'"').replace(/[‘’]/g,"'");
// Deleo sweep per QA-S1-09 / QA-NG-08: count in body.innerText EXCLUDING every .legend subtree.
window.__qa.deleoCount = () => {
  const total=(document.body.innerText.match(/Deleo/g)||[]).length;
  let leg=0;
  for(const l of document.querySelectorAll('.legend')){
    if(l.getClientRects().length>0) leg += ((l.innerText||'').match(/Deleo/g)||[]).length;
  }
  return total-leg;
};
"""
...
JS["QA-S1-09"] = r"""
// Spec: Deleo appears 0 times in document.body.innerText AFTER excluding every .legend subtree
// (State 1's legend entry 10 documents the rename and must keep saying it).
const cardBtnLit={}, cardBtnCln={}, deleoHits=[], deleoLegendOnly=[];
for(const [id,lbl] of Object.entries(window.__tabs)){
  __qa.tab(lbl);
  const c=document.querySelector('#'+id+' .ordercard');
  // 'Cancel Outbound' is a distinct specced control ([L-S3-2], QA-S3-02) — not the action button this scenario grades.
  if(c){ cardBtnLit[id]=__qa.allBtnTexts(c).filter(t=>/Outbound/.test(t) && t!=='Cancel Outbound');
         cardBtnCln[id]=__qa.cbtnTexts(c).filter(t=>/Outbound/.test(t) && t!=='Cancel Outbound'); }
  else { cardBtnLit[id]='no .ordercard'; cardBtnCln[id]='no .ordercard'; }
  const n=__qa.deleoCount();
  if(n) deleoHits.push(id+':'+n);
  const raw=(document.body.innerText.match(/Deleo/g)||[]).length;
  if(raw && !n) deleoLegendOnly.push(id+':'+raw);
}
...
return __mk2(litOK, clnOK, `dotStripped=${JSON.stringify(cardBtnCln)} DeleoOutsideLegend=${JSON.stringify(deleoHits)} DeleoInLegendOnly(allowed)=${JSON.stringify(deleoLegendOnly)}`);
"""
...
JS["QA-S1-12"] = r"""
// Current spec: the classes live on the <span> inside the <td>; asserting the cell element fails on a correct page.
__qa.tab('1 · Scan Result (Last Item Remaining)');
const spans=[...document.querySelectorAll('#s1 table.tbl tbody span.qty')];
const info=spans.map(sp=>({t:__qa.norm(sp.textContent),cls:sp.className,row:__qa.norm(__qa.clean(sp.closest('tr'))).slice(0,40)}));
const two=spans.filter(sp=>__qa.norm(sp.textContent)==='2');
const ones=spans.filter(sp=>__qa.norm(sp.textContent)==='1');
const twoOK = two.length===1 && two[0].classList.contains('qty') && two[0].classList.contains('qty-warn')
           && __qa.norm(__qa.clean(two[0].closest('tr'))).includes('100038120');
const onesOK = ones.length>0 && ones.every(sp=>sp.classList.contains('qty') && !sp.classList.contains('qty-warn'));
return __mk(twoOK && onesOK, `qty spans=${JSON.stringify(info)} twoOK=${twoOK} onesOK=${onesOK}`);
"""
...
// Current spec (rule 5): table.tbl scope is REQUIRED; a bare "#s2 tbody tr" returns 9 and is documented, not a defect.
const ok = rows.length===4 && tags.every(t=>t==='INBOUNDED') && ob && ob.classList.contains('btn-green')
        && !ob.classList.contains('btn-gray');
return __mk(ok, `scoped rows=${rows.length} (bare "#s2 tbody tr"=${bare.length}, documented rule-5 over-count) tags=${JSON.stringify(tags)} Outbound class="${ob?ob.className:'ABSENT'}"`);
"""
...
const info=tiles.map(t=>({txt:__qa.norm(__qa.clean(t)),cls:t.className}));
const find=lbl=>info.find(x=>x.txt.includes(lbl));
const exp=find('Expected Qty (Total)'), rec=find('Received (scanned)'), rem=find('Remaining'), sku=find('SKU');
const ok = tiles.length===4
  && exp && exp.txt.includes('800')
  && rec && rec.txt.includes('620') && /\bok\b/.test(rec.cls)
  && rem && rem.txt.includes('180') && /\bwarn\b/.test(rem.cls)
  && sku && sku.txt.includes('2 SKUs (1 done)');
return __mk(ok, `tileCount=${tiles.length} tiles=${JSON.stringify(info)}`);
"""
...
JS["QA-S6-08"] = r"""
__qa.tab('6 · Internal Inbound (Inbound Request)');
const a=__qa.cbtn('#s6','Confirm Full Inbound (180 remaining)');
const b=__qa.cbtn('#s6','Save Partial Inbound');
return __mk(!!a && a.classList.contains('btn-gray') && !!b && b.classList.contains('btn-line'),
  `confirmBtn=${a?'"'+a.className+'"':'NOT FOUND'} partialBtn=${b?'"'+b.className+'"':'NOT FOUND'} allS6Buttons(rule6b)=${JSON.stringify(__qa.cbtnTexts('#s6'))}`);
"""
...
const els=[...document.querySelectorAll('#s6 *')].map(e=>__qa.norm(__qa.clean(e)));
...
JS["QA-S6-10"] = r"""
// Current spec: NO visible button.flsave appears (documents demo limitation L-13); the positive assertion is QA-S6-45 [ADMIN].
__qa.tab('6 · Internal Inbound (Inbound Request)');
const inp=document.querySelector('#s6 table.tbl tbody tr:nth-child(2) input.qtyin');
if(!inp) return __mk(false,'#s6 table.tbl tbody tr:nth-child(2) input.qtyin is null');
inp.value='130'; __qa.fire(inp,'input');
const vis=[...document.querySelectorAll('#s6 button.flsave')].filter(b=>b.getClientRects().length>0);
return __mk(vis.length===0,
  `visible button.flsave count=${vis.length} (spec expects 0 — L-13: handler bound only to .shelf input and .locin, never .qtyin)`);
"""
...
return __mk(ok, `textarea="${ta?ta.value:null}" quotedFragments(case-sensitive)=${JSON.stringify(res)} footButtons=${JSON.stringify(foot)} | actualSentence="${(txt.match(/[Tt]he request list qty cell[^.]*\.?/)||[''])[0]}"`);
"""
...
const i2=txt.indexOf('2. Restock Qty'), i3=txt.indexOf('3. Location (Optional)'), i4=txt.indexOf('4. Memo (Optional)');
const order=i2>-1 && i3>i2 && i4>i3;
const loc=document.querySelector('#restockLoc');
const initVal=loc?loc.value:null;
const helper=txt.includes(__qa.nq('Default = the SKU\'s registered location (editable · updates the SKU\'s location) · required if the SKU has none · disabled when "No" is selected'));
const no=document.querySelector('input[name="restock"][value="no"]');
const yes=document.querySelector('input[name="restock"][value="yes"]');
no.checked=true; __qa.fire(no,'change');
const s1=loc?{disabled:loc.disabled,value:loc.value}:null;
yes.checked=true; __qa.fire(yes,'change');
const s2=loc?{disabled:loc.disabled,value:loc.value}:null;
const ok=order && loc && initVal==='B-01-4' && helper
      && s1 && s1.disabled===true && s1.value===''
      && s2 && s2.disabled===false && s2.value==='B-01-4';
return __mk(ok, `fieldOrder(2→3→4)=${order} #restockLoc initial="${initVal}" helperFound=${helper} afterNo=${JSON.stringify(s1)} afterYes=${JSON.stringify(s2)}`);
"""
...
return __mk(!!hit, `exactNoteFound=${!!hit} (quote-shape-normalized both sides) nearest="${near||''}"`);
"""
...
for(const [id,lbl] of Object.entries(window.__tabs)){
  __qa.tab(lbl);
  const b=[...document.querySelectorAll('#'+id+' button')].find(x=>__qa.norm(__qa.clean(x))==='🖨 Print Return Labels (2)');
  if(['s1','s1b','s2','s3'].includes(id)) present[id]=!!b;
  if(['s4','s5','s6','s6b'].includes(id)) absent[id]=!!b;
}
__qa.tab(window.__tabs.s1);
const b1=[...document.querySelectorAll('#s1 button')].find(x=>__qa.norm(__qa.clean(x))==='🖨 Print Return Labels (2)');
let opened=false;
...
const pane=dd.querySelector('.tabpane[data-pane="mentions"]');
const ph=pane?__qa.norm(__qa.clean(pane.querySelector('.paneheader')||pane)):'';
const hdr=ph.includes('Comments mentioning me · Click to open the order');   // [G-7] HUB-1
const mark=ph.includes('Mark all read') && !ph.includes('Mark all as read'); // [G-7] HUB-4
return __mk(dd.classList.contains('open') && !!men && /3/.test(men||'') && !!sav && hdr && mark,
  `open=${dd.classList.contains('open')} tabTexts=${JSON.stringify(tabs.slice(0,6))} paneheader="${ph.slice(0,160)}" HUB1=${hdr} HUB4=${mark}`);
"""

JS["QA-C-04"] = r"""
__qa.tab('1 · Scan Result (Last Item Remaining)');
document.querySelector('button[data-open="inbox1"]').click();
const pane=document.querySelector('#inbox1 .tabpane[data-pane="mentions"]');  // rule 7c
if(!pane) return __mk(false,'#inbox1 .tabpane[data-pane="mentions"] not found');
const its=[...pane.querySelectorAll('.it')];
const unread=its.filter(e=>e.classList.contains('unread'));
const orders=its.map(e=>{const m=__qa.norm(e.textContent).match(/41\d{4}/);return m?m[0]:null;});
return __mk(its.length===4&&unread.length===3&&JSON.stringify(orders)===JSON.stringify(['413865','413712','413650','413501']),
  `mentions-pane .it=${its.length} unread=${unread.length} orders=${JSON.stringify(orders)}`);
"""
...
const txt=__qa.norm(__qa.clean(dd));
const hdr=txt.includes('2 results · newest first · click to open the order'); // [G-7] HUB-5
const marks=[...dd.querySelectorAll('mark')].filter(m=>__qa.vis(m)).map(m=>__qa.norm(m.textContent));
const marksOK=marks.length>0 && marks.every(t=>/restock/i.test(t));
const orders=[...dd.querySelectorAll('.it')].filter(e=>__qa.vis(e)).map(e=>{const m=__qa.norm(e.textContent).match(/41\d{4}/);return m?m[0]:null;});
return __mk(tabsHidden && hdr && marksOK && JSON.stringify(orders)===JSON.stringify(['413650','412990']),
  `tabsHidden=${tabsHidden} headerFound=${hdr} marks=${JSON.stringify(marks)} visibleResultOrders=${JSON.stringify(orders)}`);
"""
...
const txt=__qa.norm(__qa.clean(dd));
// [G-7] HUB-6: lower-case c is normative; the pre-fix capitalized form must be gone.
return __mk(txt.includes('No matching comments') && !txt.includes('No matching Comments'), `panelText="${txt.slice(0,220)}"`);
"""
...
const mentions=dd.querySelector('.tabpane[data-pane="mentions"]');
const paneShown=mentions?__qa.vis(mentions):false;   // previously active tab (default: mentions) is shown again
return __mk(vis && paneShown, `tabsVisible=${vis} previouslyActivePaneShown=${paneShown}`);
"""
...
savedTab.click();
const mentions=dd.querySelector('.tabpane[data-pane="mentions"]');
const saved=dd.querySelector('.tabpane[data-pane="saved"]');            // rule 7c
const mentionsHidden=mentions?!__qa.vis(mentions):false;
const its=saved?[...saved.querySelectorAll('.it')]:[];
const orders=its.map(e=>{const m=__qa.norm(e.textContent).match(/41\d{4}/);return m?m[0]:null;});
const stars=its.map(e=>{const s=e.querySelector('button.star');return s?s.className:null;});
const ph=saved?__qa.norm(__qa.clean(saved.querySelector('.paneheader')||saved)):'';
const hdr=ph.includes('Saved comments · Click to open the order')      // [G-7] HUB-2
       && ph.includes('Unstar to remove from the list');               // [G-7] HUB-3
return __mk(mentionsHidden && __qa.vis(saved) && its.length===2 && JSON.stringify(orders)===JSON.stringify(['413712','412990']) && stars.every(c=>c&&/\bon\b/.test(c)) && hdr,
  `mentionsHidden=${mentionsHidden} savedVisible=${saved?__qa.vis(saved):null} entries=${its.length} orders=${JSON.stringify(orders)} starClasses=${JSON.stringify(stars)} paneheader="${ph.slice(0,160)}"`);
"""
...
const txt=__qa.norm(__qa.clean(dd));
// [G-7] HUB-1 canonical since WF-VO-1 (2026-08-03): NOT re-worded per entity type.
const hdr=txt.includes('Comments mentioning me · Click to open the order') && !txt.includes('Click to open the item');
return __mk(badge && __qa.norm(badge.textContent)==='2' && txt.includes('Inbound 202607120001') && txt.includes('Inbound 202607100005') && hdr,
  `badge="${badge?__qa.norm(badge.textContent):null}" hasReq1=${txt.includes('Inbound 202607120001')} hasReq2=${txt.includes('Inbound 202607100005')} HUB1=${hdr} paneHeader="${(txt.match(/Comments mentioning[^·]*·[^·]*/)||[''])[0]}"`);
"""

JS["QA-C-11"] = r"""
const want='🔍 Search all comments — order no. · author · text';   // [G-7] HUB-7 canonical since WF-VO-1
const got={}; let ok=true;
...
JS["QA-NG-08"] = r"""
// Deleo sweep per current spec: exclude every .legend subtree (State 1's legend documents the rename).
const files=document.querySelectorAll('input[type=file]').length;
...
for(const [id,lbl] of Object.entries(window.__tabs)){
  __qa.tab(lbl);
  const n=__qa.deleoCount();
  if(n) bad.push(id+':Deleo(outsideLegend)x'+n);
  for(const b of document.querySelectorAll('button')){
    const t=__qa.norm(__qa.clean(b));
...
JS["QA-NG-10"] = r"""
// Current spec carve-out: exactly one [G-12] deep link — #s6 .intbanner a ending ../inbound-request/index.html#reqlist.
const navAnchors=[...document.querySelectorAll('a[href]')]
  .filter(a=>{const h=a.getAttribute('href'); return h && !h.startsWith('#') && !h.startsWith('javascript:');});
const carved=navAnchors.filter(a=>a.getAttribute('href').endsWith('../inbound-request/index.html#reqlist') && a.closest('#s6') && a.closest('.intbanner'));
const violations=navAnchors.filter(a=>!carved.includes(a)).map(a=>a.getAttribute('href'));
...
return __mk(violations.length===0 && navOnclick.length===0 && carved.length===1,
  `carvedOutG12Anchors=${carved.length} violations=${JSON.stringify(violations)} navigating onclick=${JSON.stringify(navOnclick)}`);
"""
...
JS["QA-CV-18"] = r"""
// Current spec: assert CANONICAL CASING per label — do NOT assert text===toUpperCase()
// (six 'JIT (Coupang)' labels are mixed case by design and no text-transform is applied).
document.querySelector('#inexpToggle').click();
const canon=['SMART BUY','WHOLESALE','PARTNERSHIP','JIT (Coupang)'];
const bad=[]; let checked=0, jitSeen=0;
for(const id of ['s0','s1','s1b','s2','s3','s4','s5']){
  ...
  for(const e of document.querySelectorAll('#'+id+' .tag-smartbuy, #'+id+' .tag-jit, #'+id+' .tag-wholesale, #'+id+' .tag-partnership')){
    checked++;
    const cs=getComputedStyle(e); const t=__qa.norm(__qa.clean(e));
    if(t==='JIT (Coupang)') jitSeen++;
    if(cs.backgroundColor!=='rgba(0, 0, 0, 0)') bad.push(id+' bg='+cs.backgroundColor+' "'+t+'"');
    if(parseInt(cs.fontWeight,10)<600) bad.push(id+' fontWeight='+cs.fontWeight+' "'+t+'"');
    if(!canon.includes(t)) bad.push(id+' nonCanonicalCasing="'+t+'"');
    if(cs.textTransform==='uppercase') bad.push(id+' text-transform:uppercase applied to "'+t+'"');
  }
}
return __mk(checked>0 && bad.length===0, `checked=${checked} jitMixedCaseSeen=${jitSeen} violations=${JSON.stringify(bad.slice(0,12))}`);
"""
...
JS["QA-CV-11"] = r"""
// Current spec quotes the two legend sentences verbatim (rule 6 collapse) and asserts the search value.
__qa.tab('4 · Customer Return Mode');
const t=__qa.norm(document.querySelector('#s4').textContent);
const s1='If any item has restock qty > 0 but no location, the confirm button is disabled — assign a location to enable (see modal M3 — no on-screen dot)';
const s2='Searchable by last-mile (return) barcode too — the unified search auto-detects return barcodes';
const inp=document.querySelector('#s4 .search-input input');
const ok=t.includes(s1)&&t.includes(s2)&&inp&&inp.value==='10322198837710';
return __mk(ok, `sentence(i)=${t.includes(s1)} sentence(ii)=${t.includes(s2)} searchValue="${inp?inp.value:null}"`);
"""

JS["QA-GL-02"] = r"""
// Current spec: assert against the SOURCE TEXT of every inline <script>, three verbatim substrings.
const src=[...document.querySelectorAll('script:not([src])')].map(s=>s.textContent).join('\n');
const a=src.includes('function sndOutbound()');
const b=src.includes('if(/Outbound/.test(tx) && !/Cancel/.test(tx) && !/Outbounded/.test(tx))');
const c=src.includes("b.addEventListener('click',sndOutbound)");
const near=(src.match(/.{0,40}sndOutbound.{0,80}/g)||[]).slice(0,4);
return __mk(a&&b&&c, `fnDecl=${a} selectionRule=${b} addEventListener=${c} | nearby=${JSON.stringify(near)}`);
"""
...
def scen_S3_02(page):
    ...
    # rule 6b: the button is located by its rule-6b-normalized text (raw is 'Cancel Outbound2')
    ok = len(msgs) == 1 and msgs[0] == want
    return {"v": "PASS" if ok else "FAIL", "literalAlsoHeld": bool(info["literalMatch"]),
            "ev": f"button rule6b match=True (dot-stripped buttons={info['cleaned']}); dialogs={msgs!r} expected={want!r}"}
