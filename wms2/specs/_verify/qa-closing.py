...
def qa_scan_01(page):
    ...
    if inp:
        ...
        s.eq("value", "YT2618100700000004", inp["v"])
...
def qa_verdict_02(page):
    ...
    s.contains("okline contains order/tracking/status line",
               "Order 413540 · Tracking YT2618100700000004 · Prepare Shipment",
               t(page, "#s1 .okline"))
...
def qa_dup_03(page):
    ...
    if r:
        s.eq("tracking", "YT2618100700000005", r["trk"])
...
def qa_m1_02(page):
    ...
    s.contains("body tracking line",
               "Tracking YT2618100700000006 · 1 item · Status Processing",
...
def qa_del_10(page):
    ...
    page.click('.wf-tab[data-modal="m-scandel"]')
    s.ok("#m-scandel gains class open", has_class(page, "#m-scandel", "open"))
    s.eq("#scandelInfo default text", "#7 · YT2618100700000007", t(page, "#scandelInfo"))
    return s.finish()
...
def qa_chrome_03(page):
    ...
    load(page); state(page, "s1")
    s.eq("#s1 .toast span", "✓ Outbound confirmed — YT2618100700000004",
...
def qa_chrome_06(page):
    ...
    s.ok("longest tracking text fully present with no ellipsis",
         "YT2618100700000005" in trk["cells"] and not trk["hasEllipsis"], f"{trk}")
    return s.finish()
