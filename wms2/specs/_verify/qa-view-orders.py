...
import pathlib
# 레포 상대경로로 해석한다. 절대경로를 박으면 작성자 기계에서만 돌고,
# 클론한 사람에게는 전부 미기동으로 나타난다(그러면서 통과율은 그대로라 들키지 않는다).
URL = (pathlib.Path(__file__).resolve().parents[2] / "view-orders" / "index.html").as_uri()
