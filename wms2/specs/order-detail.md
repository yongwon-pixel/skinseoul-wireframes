...
Page slug: `order-detail` · Spec version 1.3 · 2026-08-08
...
The live-admin capture it reproduces is Order **#400001** (customer Jane Doe, AU, carrier YUN, 4 line items). All demo values in the wireframe are anonymized from a real order.
...
| `[L-F6]` | Order header block — `Order # 400001` + status badge + carrier badge (`YUN`) | title row | §3 furniture |
...
| **`[X-4]`** ✅ **RESOLVED 2026-08-03** | **`OUTBOUNDED` line status.** RTO's Bulk Outbound was specified to write each line `INBOUNDED → OUTBOUNDED`, while this page's rendering contract is exhaustive at two values. | `[L-10]` `Inbound Status` + `[BR-50]` | **Closed — this page won.** `ready-to-outbound` v1.2 corrected `BR-22` / `[DC-9]` to an order-level transition that writes no line status. `[L-10]` keeps two tags, `[BR-50]` stands, and no `OUTBOUNDED → INBOUNDED` reversal is needed. *(Row still read as open until 2026-08-04 — stale, not a live conflict.)* |
...
| **`[X-6]`** ✅ **RESOLVED 2026-08-03** | **Comments-hub copy** was not byte-identical across the eight specs; this page was the outlier on one string (`Unstar to remove`). | `[L-7]` — the strings specified here are now the `[G-7]` **canonical** strings (HUB-1…HUB-7), asserted byte-exactly by `QA-HUB-1`/`QA-HUB-3` | Closed by `[G-7]` v1.2, which published the canonical set; this page's wireframe hint became `Unstar to remove from the list` (HUB-3) and the spec followed in the same commit. |
...
|---|---|---|---|
| Line Items → `Tracking Number` (Inventory group) | the **purchase/inbound** parcel from the retailer or supplier to the warehouse | supplier → warehouse | `10323100000001` |
| Fulfillment Tracking → `Tracking Number` | the **outbound carrier** parcel to the customer | warehouse → customer | `34YEM000000000000000001` |
...
3. **Pane header** (`.paneheader`) — Mentions: `Comments mentioning me · Click to open the order` + a `Mark all read` action. Saved: `Saved comments · Click to open the order` + `Unstar to remove from the list`.
4. **Items** (`.it`) — `Order {no} · {author}: "{text}"` + relative time + `★` toggle. Unread items carry class `unread` (blue tint).

**Hub copy is the `[G-7]` canonical copy, byte-identical on all eight screens** `[X-6]`. The strings above (`Comments mentioning me · Click to open the order`, `Saved comments · Click to open the order`, `Unstar to remove from the list`, `Mark all read`, `{n} results · newest first · click to open the order`, `No matching comments`, placeholder `🔍 Search all comments — order no. · author · text`) are `[G-7]` HUB-1…HUB-7, and `QA-HUB-1`/`QA-HUB-3`/`QA-HUB-4` assert them byte-exactly. Until 2026-08-03 this page shipped `Unstar to remove` — the corpus's only instance of the bare form — and the spec deliberately kept it because rewriting the string here alone would have failed a `[WF]` assertion against a correct drawing. `[G-7]` v1.2 published the canonical set, so the wireframe and this spec moved to HUB-3 together in one commit.
...
**No carrier assigned — REMOVED 2026-08-04 `[G-17]`.** This branch described an order that the Order Management import created without a carrier. **The import no longer creates one**: a file containing any unconnected destination is rejected in full, so a carrier-less order cannot reach this page. The display, the Outbound/Print block, the `reason_code=no_carrier` attempt and the "who unblocks it" question (`[PD-55]`, now void) are all removed rather than left as dead paths — specified behaviour for an impossible state produces code nobody exercises and QA nobody can run. Superseded text kept in `[E-86]` and §10 so the removal is legible and does not silently return.
...
- **A manual stock adjustment on Inventory moves this number, and nothing on this page can perform one.** Inventory's `✎` editor restates a SKU's `Total` and emits `stock.manual_adjusted` (`stock-status.md` `[L-19]` `[DC-35]`); this column reads the resulting balance on its next refresh. So a line's count can change with **no event on this order at all** and with **no `DC-21`** — an `ADJUST` is a book correction, not a movement (`stock-status.md` `[BR-3]`), so `DC-21`'s `movement_type` enum is not extended and QA-DC-6's net-zero sweep is unaffected. The cell stays **read-only here**: it is on `[L-12]`'s not-editable list, it carries no `✎`, and no adjustment affordance may be added (§9.1). The reason vocabulary, the below-`Reserved` warning and the resulting `ADJUST` history row are specified once, on the screen that owns stock quantity.
...
- `stock.manual_adjusted` — restating a SKU's warehouse quantity is Inventory's `[L-19]`, and this page has no affordance for it (§9.1). Order Detail only **reads** the resulting balance into `Latest Inventory Count` `[L-10]` and receives the below-`Reserved` auto-comment (§5.3).
...
| Inventory `[L-19]` — a manual stock adjustment on a SKU this order carries | `Latest Inventory Count` refreshes on that line `[L-10]`; **no** line-item, status or inbound-state change. When the adjustment took `Total` **below `Reserved`**, an auto-comment additionally lands in the thread `[L-1]` carrying the SKU, `before → after`, the reason, the adjusting actor and the units this order holds — that comment is how this order's owner finds out, since nothing else on the page moves | `stock.manual_adjusted` (`stock-status.md` `[DC-35]`) + `comment.auto_posted` with `source = system` and the linked `[DC-35]` id as its origin (`stock-status.md` `[DC-37]`) — `source = system` is the canonical qualifier every auto-comment in the corpus carries, the same value `DC-32` records, so nothing here forks it. Both are written by Inventory. Order Detail renders them and writes nothing — in particular **no `DC-21`**, because an `ADJUST` is a book correction, not a movement (`stock-status.md` `[BR-3]`) |
...
- **Inventory.** Consumes `DC-21`; `Latest Inventory Count` reads the current balance `[L-10]`. Inventory's M4 release path can reverse the same reservation this page's Cancel Inbound reverses `[PD-45 · OWNER-PENDING]` — the server must guarantee one reversal per line, the second attempt being a stale-entity rejection, not a second movement. Inventory is also the **only** writer of a manual `Total` restatement `[L-19]`: `stock.manual_adjusted` moves the balance this column reads, and a below-`Reserved` adjustment additionally posts a `comment.auto_posted` onto each holding order, which lands here (§5.3). Order Detail consumes both and originates neither.
...
| **E-86** | **UNREACHABLE 2026-08-04 — ID kept so cross-references resolve.** The order has **no carrier**. | **Cannot occur under `[G-17]`**: the Marketing Order Import rejects any file with an unconnected destination, and no other surface creates an order without a carrier, so this page never receives one. *(Superseded behaviour, 2026-08-03: Outbound and Print disabled with that state as the visible reason `[BR-46]`, attempts persisting `DC-9` `reason_code=no_carrier`, and `[PD-55 · NO-DEFAULT]` open for who unblocks it.)* `[BR-46]` and `QA-OUT-11` are retired with it. |
...
- **And** `.wf-bar .hint` reads exactly `v1 — Based on the live admin (Order #400001) · Purple numbers = new/changed annotations`
...
- **Then** `#inbox1 [data-pane="saved"]` is visible, its `.paneheader` reads `Saved comments · Click to open the order` with `Unstar to remove from the list`
...
- **When** I type `dean`
- **Then** the header reads `2 results · newest first · click to open the order` and the items appear in this order: `Order 400001` first, `Order 407301` second.
...
- **Then** its Inventory cells read, in order: `0`, `INBOUNDED`, `JIT (Coupang)`, `12101316464794`, `2026-06-30`, `₩17,100`, `10323100000001`, and a link reading `coupang…7923`
...
- **And** its single row reads `YT2618100700000001` / `34YEM000000000000000001` / `DELIVERED` (in a `.st-delivered` chip) / `Shipment information received` / `6/30/2026, 8:55:53 PM` / `7/13/2026, 10:10:32 AM`
...
- **Then** the line-item `Tracking Number` for SKU `100043697` is `10323100000001`
- **And** the Fulfillment Tracking `Tracking Number` is `34YEM000000000000000001`
...
- **Then** `#st-normal .otitle h2` reads exactly `Order # 400001`
...
- **And** the billing panel includes `Email` = `jane.doe@example.com` and `Phone` = `+61400000001`.
...
- **When** I change `City` from `Sydney` to `Bondi` on the shipping address and save
- **Then** `order.address_edited` persists with `scope=shipping` and `field_diff{city:{old:"Sydney", new:"Bondi"}}` (`DC-7`), and a green toast reads exactly `Shipping address updated`
...
4. **QA-INB-1 … QA-INB-5, QA-OUT-1 … QA-OUT-3, QA-STA-1 … QA-STA-3, QA-STA-5, QA-STA-6, QA-EDIT-1 … QA-EDIT-5, QA-PRT-1, QA-PRT-2** — per-legend render and state-tab assertions. **`QA-STA-4` is part of the `[WF]` run again** since `[OD-WFX-1]` was applied on 2026-08-03 and the wireframe gained the outside-click handler.
...
| **Line-based location filter and audit-mode visibility `[G-14]`; JIT residual stock; manual stock adjustment `[L-19]`** | Inventory (stock-status). Order Detail shows `Latest Inventory Count` as a read `[L-10]`. Inventory's M4 reservation release is the sibling of this page's Cancel Inbound `[PD-45 · OWNER-PENDING]` — cross-referenced in §6.5, specified there. **Restating a SKU's stock quantity is Inventory's too** — the `✎` on a `Total` cell, its four-value reason list and its below-`Reserved` warning exist only there; this page has no adjustment affordance and may not gain one `[L-10]`. |
...
- **`[OD-WFX-1]` (this page) — APPLIED 2026-08-03:** the **Change Status dropdown closes on an outside click**. `order-detail/index.html` previously had a backdrop handler for the modal but no document-level handler for the dropdown, so it stayed open (measured 2026-08-03); the wireframe now carries the document-level handler for both `#statusdd` and `#statusddH`, closing them with no status change and no event. The specified behavior `[L-8]` step 1 `[E-46]` never changed. `QA-STA-4` is back at `[WF]` tier with no edit to the scenario body, restored to the §8.3 step-4 run list, and the §8.1 split is 69 `[WF]` / 92 `[ADMIN]`. Registered in `_wireframe-fixes.md` §I.
...
Every decision that shaped this screen, 2026-07-09 → 2026-08-08, including reversals and removals. Commit hashes are from `yongwon-pixel/skinseoul-wireframes`.
...
| 2026-07-14 | **Real-capture rework** against the live admin, Order #400001: horizontally scrolling Line Items, sourcing-route badges, upgraded comments, **Actor Log introduced (new)**, button enablement rules | commit `9844fe0` |
...
| 2026-08-08 | **Spec v1.3 — Inventory's manual stock adjustment propagated inward; this page offers nothing new.** Inventory gained a per-row `✎` on the `Total` cell (`stock-status.md` `[L-19]` / `[L-20]`, `_global-rules.md` v1.6), and the whole consequence here is a **read**: `Latest Inventory Count` can now move with no event on this order at all, because `stock.manual_adjusted` restates the balance the column reads `[L-10]`; a below-`Reserved` adjustment additionally lands an auto-comment in this order's thread, recorded as a fifth row of §5.3 and as a boundary in §6.5. **No `DC-21` is written for it** — an `ADJUST` is a book correction, not a movement (`stock-status.md` `[BR-3]`) — so `movement_type` is not extended and QA-DC-6's net-zero sweep is unaffected. The negative is registered in §9.1: restating a stock quantity is Inventory's, this page has no adjustment affordance and may not gain one, and `Latest Inventory Count` stays on `[L-12]`'s not-editable list. **No id was minted, renumbered or retired** — the 92-edge-case, 37-event and 16-NON-event counts are unchanged, and no QA scenario needed rewriting | `stock-status.md` §3.22 / §3.23; `_global-rules.md` v1.6; owner decision 2026-08-08 |
