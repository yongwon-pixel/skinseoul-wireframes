...
- **The import must never dead-end on one bad row's carrier.** A country with no connected carrier is flagged amber inline and the other rows still import (BR-20). The amber string *is* the instruction (`Not connected — contact the Fulfillment Center`), so the operator does not have to work out who to ask.
...
- **Feedback:** a green toast top-right `[G-2]`:
  ...
  - Subtext (byte-exact): `Carrier auto-assigned per country · {u} not connected — flagged to contact Fulfillment Center` (demo: `Carrier auto-assigned per country · 1 not connected — flagged to contact Fulfillment Center`)
...
| **BR-20** | **An unconnected carrier never blocks the batch.** The order is created and carries a persistent `carrier_unresolved` flag; the amber cell text is the resolution instruction. | Import runs under time pressure; one unroutable country must not stop eleven shippable orders. | 2026-08-03 |
...
| **E-7** | Country valid but no connected carrier | **Not an error.** Amber cell `Not connected — contact the Fulfillment Center`; Confirm stays enabled; order is created flagged (BR-20). `[DC-11]` |
| **E-8** | **Every** row unconnected | Import still confirmable; toast subtext reports the full count |
...
Then the pane `[data-pane="saved"]` is displayed with header `Saved comments · Click to open the order` and hint `Unstar to remove from list`, containing exactly one row, for `Order MKT-40218`
