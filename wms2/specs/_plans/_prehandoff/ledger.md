...
| F-4 | MAJOR | QA재실행 | **OM 결함-문서화 시나리오 8건이 수리 전 상태를 어서션** (QA-IMP-35·SMP-19/30/31/33·CMT-20·GBL-09/10) — a518f5a가 WF-15~21을 화면에 적용했으나 스펙 §2.4/§8.0 rule 4·7과 이 8건은 '[WF-n · proposed]' 기준 그대로. 실측은 전부 스펙 §3 제품 요건과 일치(colspan 7·확인창·토스트 스택·Esc 등) | (internal run) 저널 order-management (2런 안정) | OPEN |
| F-5 | MAJOR | QA재실행 | **VO QA-S6-07 "유일 예상-실패" 선언 스테일** — WF-1(캐리어 배너)이 이미 수리돼 PASS함. §2.4 WF-1 행·§8.0 'expected to FAIL today' 조항·배너 문구 북키핑('· Carrier is not recorded' 추가형) 정비 필요 | (internal run) 저널 view-orders NOTE 1 | OPEN |
| F-6 | MINOR | QA재실행 | TM QA-EMPTY-01의 >300px 절이 inner .mockwrap 앵커로는 **원천 불충족**(기준선 inner=260px, outer=520px가 .mock min-height로 유지) — outer로 재앵커 필요 | (internal run) 저널 tracking-missing | OPEN |
| F-7 | MINOR | QA재실행 | OD §8.1 표의 [WF]=68 잔존 — v1.3 델타(69)로 갱신 안 된 표 1곳 | (internal run) 저널 order-detail | OPEN |
| F-8 | MINOR | QA재실행 | INV QA-GLB-08 clause 3 자기모순(activeElement 어서션 vs 자체 정의문) — 정의문 기준으로 재서술 | (internal run) 저널 stock-status | OPEN |
| F-9 | NOTE | QA재실행 | **[WF] 전량 재실행 654건: 645 PASS · 9 FAIL(전건 spec-stale) · 와이어프레임 결함 0** · 러너 8종=_verify/prehandoff/ · 전 에이전트가 기준선 byte-identical 사전 검증 | (internal run) 완주(2.1M tok, 21분) | VERIFIED |
...
| F-32 | 검증대기 | R3 dev-eyes | **개발자 시선 감사 9/9 — 질문 236건(BLOCKER 47·MAJOR 125·MINOR 64)**, 판정 BLOCKED 6 / BUILDABLE-WITH-QUESTIONS 3. 콘텐츠 분류 평균 **규범 78.7% · 참고 12.6% · 과정잔여물 8.6%**(감독 추정 30%보다 훨씬 낮음). 이동 후보 74건·모호 [ADMIN] QA 118건. **BLOCKER 47건은 반증 전이라 미확정** | (internal run)(2.4M tok) | OPEN |
...
### R5-D 노션 전면 재게시 결과 (10/10, (internal run), 3.0M tok)
| 지표 | 결과 |
|---|---|
| 헤딩 수·순서 | **10/10 페이지 정확히 일치** |
| 표 개수 | **10/10 일치**(closing 37/36 차이는 레포 결함이었고 수리함 — 표 중간 빈 줄) |
| 절단 | **0건** |
| 셀 분할(이전 라운드의 주 손상) | **0건** — 정본 표기 정리(A 수리)가 유효했음이 실증됨 |
| **날조 문장** | **0건** — 페이지별 20단어↑ 문장 30개 표본을 레포와 대조, 전건 소재 확인 |
| 정본 문자열 스팟 | 페이지당 5~10개 바이트 일치 |

**잔여(플랫폼 한계, 수리 불가)**: 노션이 표 셀 안에서 닫는 백틱 직후 `+`를 불릿 글리프로 직렬화한다. 이스케이프 불가가 실증됨(노션 enhanced-markdown의 escapable set에 `+`가 없고, `\+` 발행 시 재fetch 결과가 verbatim과 바이트 동일). **표 밖에서는 정상 렌더**되므로 표 셀 한정 플랫폼 결함. 단어 추가·삭제 없음. 의미가 실제로 바뀌던 유일 케이스(`` `6`+ `` = "6 이상")는 정본에서 문구 교체로 해소.
**부수**: 볼드 마커가 인라인 코드 주변에서 재분배됨 — 정규화 후 차이 0.

**재게시 검증이 잡아낸 레포 결함 2건**(수리·커밋 0380eea): closing §10 표 분단 빈 줄 · OM `` `6`+ `` 의미 훼손.
