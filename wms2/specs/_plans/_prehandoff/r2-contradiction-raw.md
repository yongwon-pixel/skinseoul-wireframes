...
### [C-40] MINOR — 동일 주문·동일 SKU 픽스처 드리프트: stock-status는 Order 400001에 SKU 100004819의 INBOUND +6(PENDING) 이동을 단언하지만 order-detail의 Order 400001은 4개 라인(100005104/100043697/100005088/100012534)·수량 각 1로 100004819를 포함하지 않으며, SKU 100005104의 KR 명칭도 '포어레미디 리뉴잉 폼'(stock-status) vs '포어레미디 리뉴잉 폼 클렌저'(order-detail)로 갈린다.

- `wms2/specs/stock-status.md`: "the only `tr.pending` row is the `INBOUND +6` row (tracking `12101316464794`, carrier `Coupang`, order `400001`, auditor `Miranti`)"
- `wms2/specs/order-detail.md`: "The live-admin capture it reproduces is Order **#400001** (customer Jane Doe, AU, carrier YUN, 4 line items)."
