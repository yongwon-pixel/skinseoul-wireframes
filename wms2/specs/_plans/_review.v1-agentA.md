...
Verdict rule (per supervisor): **2026-08-03 decisions always win**; wireframe legend notes dated 08-03 beat older wireframe body text; `_inputs/slack-routing.md` (CONFIRMED 08-03) beats the "pending" wording in `global-rules-draft.md` / `decision-sources.md`.
...
- **[C-2] Comment-mention channel.** `global-rules-draft.md` G-7 says channel "pending"; decision-sources item 12 says "channel decision pending owner". `_inputs/slack-routing.md` says **CONFIRMED: #fulfillment-admin-comments (C0BMGEWM5QA), owner, 2026-08-03**. **Verdict: routing file wins.** All 16 plans already agree. Specs cite the confirmed channel + ID; G-7 text amended (§5 below).
...
6. **Slack notation**: `#channel-name` with ID in parentheses on first mention per spec — `#fulfillment-admin-comments` (`C0BMGEWM5QA`), `#unrecognized-tracking`, `#wholesale-ops`, `#partnership-kr`. Payload fields verbatim from `_inputs/slack-routing.md`. Never write "pending" for confirmed rows.
