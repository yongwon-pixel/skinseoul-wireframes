...
- Mentions pane header: `Comments mentioning me · Click to open the order` with a right-aligned `Mark all read` action [DC-4]. On use it shows the green toast `✓ All mentions marked as read` **(spec-defined)** because the change spans rows the operator may not be looking at [G-2] [BR-34]. (Inert in the wireframe — `[ADMIN]`.)
- Saved pane header: `Saved comments · Click to open the order` with `Unstar to remove from the list`.
...
**Sample lines.** Internal picking artifacts list **which** sample set and **how many** per [G-13] `[PD-36 · OWNER-PENDING]` [BR-21]. The wireframe M1 has no sample rows — registered as **WF-9**, whose fix is explicitly conditional on the owner approving PD-36 *and* answering PD-51 (where the sample-set contents are defined, §9.2 OQ-1). Until both land, the picker cannot pick the sample even though the carrier-facing label already says `(+ sample set)`. A sample SKU with no registered location follows the unknown-marker rule rather than being dropped [E-66].
...
| **E-28** | **Sample-set order in the picking flow** [G-13]. | The picking list must show **which** sample set and **how many** `[PD-36 · OWNER-PENDING]` [BR-21]. Wireframe gap **WF-9**; blocked on PD-36 approval and PD-51 (definition source, §9.2 OQ-1). Until then the picker has no instruction for the sample even though the carrier-facing label already appends `(+ sample set)`. |
...
**QA-M1-03 `[WF]` — Row count and location-ascending sort**
- **Then** `#m-pick .picktbl tbody tr` has exactly **4** rows and the first-column values, in document order, are `A-02-13`, `A-03-02`, `B-01-07`, `B-02-11` — strictly ascending [BR-7].

**QA-M1-04 `[WF]` — SKU column values**
- **Then** the second-column values, in document order, are `100039958`, `100039420`, `100035912`, `100013286`, and each matches the `.skupill` of the corresponding main-table item line.

**QA-M1-05 `[WF]` — Korean product names and bold multi-unit quantity**
- **Then** row 2 `Product` equals `마데카 크림 타이트닝`, row 3 equals `세라마이드 아토 컨센트레이트 크림`, row 4 equals `마데카 크림 타임 리버스`; row 1 `Qty` is `5` rendered inside a `<b>` element and its `Order` cell reads `422221`; rows 2–4 `Qty` are `1` and not bold [G-6] [BR-5].
...
| **QA-E-20** | `[E-28]` | `[ADMIN]` · negative | **Given** an order carrying an assigned sample set. **Then** today the picking list contains **no** sample row (wireframe gap **WF-9**), and this scenario stays **blocked** until `[PD-36]` is approved and OQ-1 (`[PD-51]`) is answered; on unblocking it asserts one row naming the sample set and its quantity [BR-21] [G-13]. |
...
- **What this spec does.** States the output requirement (M1 and the printed picking list must render sample lines), states the snapshot and unknown-location behavior around it [E-66] [E-31], and stops. It does not invent a definition source, a configuration screen, or an owner.
- **Blocking.** Wireframe fix **WF-9** (adding sample rows to M1) is explicitly gated on this answer **and** on owner approval of `[PD-36]`. QA-E-20 is written and marked blocked. Until both land, the picker has no instruction for a sample the carrier-facing label already promises.
- **Owner:** product owner. **Also affects:** `order-management.md`, `order-detail.md`, `shipping-label` (Phase 3-1).

**OQ-2 — "Not connected — contact the Fulfillment Center" orders: what unblocks them, and who owns it?** `[PD-55 · NO-DEFAULT]`
- **The gap.** Such an order is created and does appear in this list, but no screen offers a manual carrier assignment and no Slack route exists for the follow-up. Deciding this would invent both a UI affordance and an owner.
- **What this spec does.** States that the flagged state is rendered and persisted; that the order behaves like any other row for selection, picking, and Bulk Outbound; and that **label printing alone** is refused and audited [E-61] [DC-28]. It does **not** specify an unblocking action on this page and wires **no** Slack alert §6.1.
- **Owner:** product owner + fulfilment lead. **Also affects:** `order-management.md`.
