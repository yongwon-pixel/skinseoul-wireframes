...
**Spec version:** 1.3 · **Date:** 2026-08-08 · **Template:** `_inputs/spec-template.md` (10 sections, in order)
...
|---|---|---|
| **WF-9** | M1 picking list had no sample-set rows, while [G-13] requires internal picking artifacts to carry one. **Resolved** — the row was added to the wireframe 2026-08-03, `[PD-51]` fixed its content ("sample set" only, no type/qty) and `[PD-36]` was **owner-decided 2026-08-04** in the affirmative. Nothing is conditional and nothing is owed. | §3.15 (`[L-M1]`), [BR-21] |
...
**The `Inventory` predicate is route-based, not stock-based — so stock that disappears under a ready order changes nothing on this screen.** Inventory's manual stock adjustment (`stock-status.md` `[L-19]`) can take a SKU's `Total` **below `Reserved`**, which means units this pool has already promised are physically gone; the order stays here and the picker will walk to the rack and find nothing. **What this screen shows in that case is: exactly what it showed before.** Every element a reader might expect to react is defined on something else:

- **Tab membership and the parenthesised counts** are decided by origin and route (regular / MKT / JIT), never by on-hand quantity, so the order does not leave `Inventory` and `Inventory + Marketing + JIT = All` still holds.
- **The Pick Locations cell** keeps its rack `.locpill`. An adjustment restates a quantity and does not touch the SKU's registered location, and the amber `Not inbounded` pill reports *inbound state*, not stock. `[L-12]`'s four cell variants are complete; **no fifth variant, no colour change and no new pill** is introduced [BR-23].
- **`Total Items`** is order size, not stock [L-13], so it does not move either.
- **No badge.** The only two badges on this page remain `Fully Inbounded` [L-7] and `MKT` [L-8] §1.4.
- **Bulk Outbound still includes the order.** Its lines are still `INBOUNDED`, and [DC-21]'s `exclusion_reason` enum stays at its five values §3.4 step 6 — **no stock-shortage member is added**, and an implementer must not invent one.

**The notification path is the auto-comment, and it is the only one.** A below-`Reserved` adjustment auto-comments **each affected order** and notifies `#fulfillment-admin-comments` (`stock-status.md` `[L-19]` `[DC-37]`; [G-7]). Because comments on this page are views over the shared corpus §5.3, that comment reaches the operator here through affordances that already exist: the row's `💬` unread badge and its inline panel `[L-9]`, and the Comments hub `[L-10]` — including hub search, which finds it whether or not the order is in the current view [E-38]. Nothing else on the row changes, which is exactly why the comment is load-bearing rather than supplementary.

**Acting on it is not this page's job.** Re-sourcing, holding or cancelling the order happens on Order Detail through the `[L-F5]` doorway §1.4, and correcting the stock figure happens on Inventory. This page's contribution is to keep showing the order truthfully — it is still ready by every criterion this screen measures.
...
**Sample lines.** Internal picking artifacts render the sample as a single **"sample set"** line per [G-13] [BR-21] — **v1: "sample set" only**, no sample type and no per-type quantity (`[PD-51]` owner-decided 2026-08-03; distinguishing which sample and how many is follow-up work for when sample types are introduced). **WF-9 was applied to the wireframe on 2026-08-03** (owner-approved batch): M1 now renders one amber-tinted `sample set` row for order `422165`, carrying a `Sample` marker in place of a location and `×1` as the quantity. `[PD-36]` was **owner-decided 2026-08-04** in the affirmative, ratifying that row; `[PD-51]` fixed the content (the line reads "sample set"). A sample SKU with no registered location follows the unknown-marker rule rather than being dropped [E-66].
...
| **BR-21** | Internal picking artifacts (M1 and the printed picking list) render a single **"sample set"** line — v1: "sample set" only, no type/qty breakdown (`[PD-51]` owner-decided 2026-08-03) [G-13]. That the line exists at all is `[PD-36]`, **owner-decided 2026-08-04** in the affirmative. | The carrier-facing label only appends `(+ sample set)`; the internal line tells the picker a sample set must be picked. Wireframe gap **WF-9** — **applied 2026-08-03**; M1 renders the sample-set row. | 2026-08-03 (content), 2026-08-04 (existence) |
...
| **Sourcing-route badge column** | No SMART BUY / JIT (channel) / WHOLESALE / PARTNERSHIP / OTHER label on this page; those render on View Orders and Inventory. **Order Detail does not render `OTHER (channel)`** — [G-5], published 2026-08-04 (`CP-3`); the earlier wording on this page named it and was wrong | 2026-08-03, corrected 2026-08-04 |
...
Exactly **one** Slack route originates on this page. Payload fields are verbatim from `_inputs/slack-routing.md` (CONFIRMED 2026-08-03).
...
Future routes are decided per feature at development time (`_inputs/slack-routing.md` last row).
...
| **Order Management** sample assignment [G-13] | The order's sample set, which internal picking artifacts must render `[PD-36]` (owner-decided 2026-08-04); Order Detail shows the same detail on its line items `[PD-27 · OWNER-PENDING]` |
| **Inventory** | Location codes and staging shelves rendered in `[L-12]`; one location per SKU and one SKU per location `[PD-46 · OWNER-PENDING]`. Also, when a manual stock adjustment (`stock-status.md` `[L-19]`) takes a SKU **below `Reserved`**, an **auto-comment on each affected order** (`stock-status.md` `[DC-37]`) — which is the *whole* of what arrives here in that case: no row change, no tab or count change, no badge and no eligibility change §3.14 |
...
7. **Picking-list layout** follows §3.15: Location / SKU / Product (Korean + bold EN brand) / Qty / Order, location-ascending with the [BR-20] tie-break, with sample lines last and outside the ordering [BR-21] `[PD-36]`.
...
| **E-28** | **Sample-set order in the picking flow** [G-13]. | The picking list renders a single **"sample set"** line — v1: "sample set" only, no type/qty breakdown (`[PD-51]` owner-decided 2026-08-03) `[PD-36]` (owner-decided 2026-08-04) [BR-21]. Wireframe gap **WF-9** — **applied 2026-08-03**; M1 renders the sample-set row, asserted by QA-M1-03/04/05/06. |
...
| **E-66** | **A sample set is assigned whose sample SKU has no registered location.** | The sample line still appears on the picking list, with the unknown marker in place of a location `[PD-36]` [BR-21]. Dropping it would silently un-promise something the carrier-facing label already promised [G-13]. |
...
| **QA-E-20** | `[E-28]` | `[ADMIN]` · negative | **Given** an order carrying an assigned sample set. **Then** the picking list contains exactly one `sample set` row for that order — **unblocked 2026-08-03** when `WF-9` was applied and `[PD-51]` answered the content question, and fully ratified 2026-08-04 by `[PD-36]`. The `[WF]` half is covered by QA-M1-03/04/05/06; this `[ADMIN]` scenario asserts the production behavior — exactly one row, reading `sample set`, carrying its quantity [BR-21] [G-13]. |
...
- **Effect on this page.** The picking list's sample line reads "sample set" [BR-21]; wireframe fix **WF-9** (adding sample rows to M1) was applied 2026-08-03 and is no longer gated on anything — `[PD-36]`, the remaining approval, was **owner-decided 2026-08-04** in the affirmative. QA-E-20 is unblocked. Snapshot and unknown-location behavior [E-66] [E-31] unchanged.
- **Owner:** decided by the product owner 2026-08-03. **Also affects:** `order-management.md`, `order-detail.md`, `shipping-label` (Phase 3-1).

**OQ-3 — How is an *order* classified as JIT when its lines carry different sourcing routes?** `[G-5]` — **RESOLVED, OWNER-DECIDED 2026-08-04**
- **The answer: any line.** An order is JIT when **at least one** of its lines is JIT-sourced. The badge, tint, view tab and sort are operational signals meaning *"this order is waiting on a purchase that has not happened yet"*, and one unsourced line blocks the whole order from shipping — an all-lines rule would hide mixed orders that are equally blocked. **Accepted consequence:** the JIT tab is a picking/blocking queue, not an accounting segment, so it lists mixed orders too. Decided together with `stock-status.md` OQ-2 (the stock half of the same axis), which resolves to the FIFO active receipt.
...
**OQ-2 — "Not connected — contact the Fulfillment Center" orders: what unblocks them, and who owns it?** `[PD-55]` — **VOID 2026-08-04. The question has no subject left.**
- **What happened.** On 2026-08-04 the owner ruled that such an order must never exist: `[G-17]` refuses the whole Marketing Order Import when any destination country has no connected carrier, so no `carrier_unresolved` order is created. There is nothing left to unblock. This is an **invalidation, not an answer** — in the manner of `[PD-43]` — and it cannot be reversed by reversing an unblocking choice; reinstating it would mean reinstating carrier-less orders, which `[G-17]` forbids.
- **Superseded answer (2026-08-03, kept so the reversal is visible):** manual coordination — contact the fulfillment person in charge via Slack; v1 shipped no in-admin carrier-assignment UI.
- **Effect on this page.** The carrier-less branch is removed: `[E-61]`, the `no_carrier` refusal reason on `[DC-28]`, and the label-exclusion subtext for it describe a state that can no longer occur. **Removed rather than left as dead paths** — a specified behaviour for an impossible state produces code nobody exercises and QA scenarios nobody can run.
...
| Item | Status |
|---|---|
| **WF-9** — add sample rows to the M1 picking table | **Done** — applied to the wireframe 2026-08-03; both gates cleared (OQ-1 / `[PD-51]` 2026-08-03, `[PD-36]` 2026-08-04). Not owed; listed only so the register and this table agree |
...
| 2026-07-23 | **REVERSAL — sample assignment reinstated as simple ON/OFF** with multiple, possibly overlapping periods and no sample-type selection. The 2026-07-22 "removed" note was stale within a day and is superseded. | Owner decision; handoff note G corrected 2026-08-03 | Reaches this page through [G-13] → [BR-21] → M1 sample rows `[PD-36]` (owner-decided 2026-08-04) |
...
| 2026-08-03 | **Comment @mention channel confirmed: `#fulfillment-admin-comments` (`C0BMGEWM5QA`).** Supersedes the "pending owner decision" wording in the draft global rules and in `decision-sources.md` item 12. | Owner; `_inputs/slack-routing.md`; adjudication C-2 | §6.1, [G-7] |
...
| 2026-08-04 | **`[PD-36]` owner-decided in the affirmative — the picking list carries a sample-set line.** Ratifies the WF-9 row applied 2026-08-03 and ends the state where §2.4, §9.4, [DC-5], §6.5, §7 and OQ-1 still read "conditional / blocked / once PD-36 lands" while §3.15 read "settled" — a contradiction the pre-handoff review registered as C-15/C-85. Content unchanged ("sample set" only, `[PD-51]`). | Owner decision | §2.4, §3.15, [BR-21], [DC-5], §6.5, §7 ([E-28], [E-66]), §9.2 OQ-1, §9.4; QA-E-20 and QA-M1-03/04/05/06 unaffected in behavior |
| 2026-08-03 | Adjudication **C-8**: [G-13] doctrine wins — internal picking artifacts list which sample and how many, so the picking list gets sample lines. *(The "which and how many" half was narrowed the same day by `[PD-51]` to "sample set" only; the existence half became `[PD-36]`, decided 2026-08-04.)* | `_review.md` §1; `[PD-36]` | [BR-21], **WF-9** (applied 2026-08-03), QA-E-20 |
...
| 2026-08-08 | **Spec v1.3 — a below-`Reserved` manual stock adjustment is answered with a stated non-reaction.** Inventory gained a per-row manual adjustment on the `Total` cell (`stock-status.md` `[L-19]`, `_global-rules.md` v1.6) which is **allowed to take `Total` below `Reserved`**, so an order can now sit in the `Inventory` tab holding units that are physically gone. §3.14 states what this screen shows in that case — **exactly what it showed before** — and states it element by element so no half of it is later "fixed": the tab predicate is route-based so membership and the counts do not move; the Pick Locations cell keeps its rack pill because an adjustment restates a quantity, not a location, and `[L-12]` keeps its **four** variants; `Total Items` is order size; the two badges stay `Fully Inbounded` and `MKT`; and Bulk Outbound still includes the order because its lines are still `INBOUNDED` and [DC-21]'s `exclusion_reason` enum stays at **five** values. **No badge, pill, tint, tab, state or exclusion reason was invented** — the screen has no stock-quantity input to react to. The notification path is the auto-comment Inventory posts on each affected order, surfacing through the affordances that already exist (`💬` unread badge and panel `[L-9]`, hub `[L-10]`), recorded as an inbound link in §6.2. **No id was minted, renumbered or retired** — the 80-edge-case count and the 201-scenario §8.0 table are unchanged | `stock-status.md` §3.22; `_global-rules.md` v1.6; owner decision 2026-08-08 | §3.14, §6.2 |
