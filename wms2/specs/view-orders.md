...
- **L-7** Checkboxes, `🔍 Search`, `🖨 Print`, `Export by date`, `Post`, and `Mark all as read` are inert.
...
- **Behavior — resolution precedence (fixed order, first match wins):**
  1. **Inbound-request tracking number** → **State 6**, always, never a customer-order state. Every tracking number registered on the request matches, including the 2nd and 3rd of a split shipment `[G-10]`.
...
- **Behavior:** a green banner (`.donebanner`) reading `✓ Full Inbound Complete — Inbound No. {n}` with a key/value row: `Received {received} / {expected} ({n} SKUs)` · `Inventory updated ({loc} · {loc})` · `Inbound Request List switched to INBOUNDED` · `Received Date {date} {time}`.
- **Correction:** the shipped wireframe appends `· Carrier recorded automatically`. That clause is **wrong** and must not be implemented — defect **WF-1**. The corrected banner ends at the Received Date; if a clause is wanted in its place, use `Carrier is not recorded`.
...
| **BR-10** | After Outbound, every per-line `Cancel Inbound` is disabled; `Cancel Outbound` must run first. | Keeps inventory arithmetic reversible and prevents an order in `prepare shipment` from holding a PENDING line. | proposed 2026-07-09, adopted 2026-08-03 `[PD-10 · OWNER-PENDING]` (defect **WF-3**) |
...
| **BR-17** | A tracking number registered on an Inbound Request always opens State 6, and **every** tracking number of a split shipment matches the same request; partial arrivals accumulate. | Split shipments are normal; one request must survive several boxes `[G-10]`. | 2026-08-03 |
...
| **BR-24** | **Received Date is recorded automatically. Carrier is not recorded at all** — no field, no column, no capture, on this page or on the Inbound Request. | Reverses the 2026-07-09/2026-07-27 "Received Date + Carrier automatic" note. Carrier is not reliably derivable at scan time and a wrong carrier is worse than none. | 2026-08-03 `[PD-9 · OWNER-PENDING]` (defect **WF-1**) |
...
|---|---|---|---|---|
| 1 | **M2b** `Send to Missing Tracking List` `[L-M2b]` | `#unrecognized-tracking` (no channel ID is published in `_slack-routing.md`; it is resolved at wiring time) | tracking no., product, qty, memo, registrant, suspected orders | none (channel post) |
...
- `Outbound` → **Ready to be Outbounded** (the order leaves the ready list) and the order's status everywhere.
...
| **E-43** | Comments hub with empty Mentions / Saved, or a search with no hits | Explicit empty-state copy per tab; search renders `No matching Comments`. Never a blank dropdown. |
...
**QA-S6-07 [WF]** `[L-S6b-1]` `[BR-24]` **WF-1** — *negative, known wireframe defect*
Given State 6b, Then `#s6b .donebanner` must **NOT** contain the string `Carrier recorded automatically`. **This assertion is expected to FAIL on the live wireframe today** — that failure is defect **WF-1** and must be filed against the wireframe, never against this spec. The correct banner ends at `Received Date 07-26 14:02`.
...
Given tab `6 · Internal Inbound (Inbound Request)` is active, When I open `#inbox6`, Then the badge reads `2`, the entries reference **inbound requests** (`Inbound 202607120001`, `Inbound 202607100005`), and the pane header is `Comments where I'm tagged · Click to open the item` — proving inbound requests are a first-class commentable entity.

**QA-C-11 [WF]** `[L-S1-3]`
Given each of States 0, 1, 1b, 2, 3, 4, 5, Then a `.inboxdd` with a `.csearch input` whose placeholder is exactly `🔍 Search all Comments — order no · author · text` exists — the hub is present on every state, not only State 1.
...
|---|---|---|
| **R-1** | **Carrier auto-record.** 2026-07-09 / 2026-07-27: "Received Date **and Carrier** are recorded automatically on confirm" → **2026-08-03: automatic Carrier recording is NOT supported.** Received Date remains automatic (it feeds the sheet sync); Carrier has no field, no column, no capture anywhere. The State 6b banner still carries the stale clause — defect **WF-1**; the State 6 legend footer is already correct. | `[BR-24]` `[L-S6-F]` `[L-S6b-1]` `[PD-9 · OWNER-PENDING]`; `_review.md` C-1 |
