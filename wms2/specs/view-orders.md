...
**Spec version:** 1.4 (Inventory manual-adjustment propagation) · **Written:** 2026-08-03 · **Last amended:** 2026-08-08 · **Template:** `_inputs/spec-template.md` v1
...
- **Existing inventory vs. a manual stock adjustment that empties the SKU.** Inventory can now restate a SKU's warehouse `Total` directly, including down to `0` (`stock-status.md` `[L-19]`, 2026-08-08). The two meet in both directions and neither is a defect:
  - **Adjustment after the pick.** The deduction already happened. A later adjustment **does not** reverse it, re-route the line, clear the `row-exist` tint or blank the `Location` value `[L-S1-17]` — the line records what was actually taken, and `Location` is a **locator, never a quantity**, so it keeps rendering the rack code at zero stock. The adjustment is an `ADJUST` **book correction, not a movement** (`stock-status.md` `[BR-3]`), so it writes **no** `inventory.stock_applied` `[DC-39]` and its `origin` enum gains no member; nothing on this page is re-rendered by it.
  - **Adjustment before the pick.** A SKU already at `0` has nothing for Existing inventory to deduct. **This page performs no stock-availability check and renders no stock quantity anywhere** — the shortage is therefore not visible here, and no gate, warning or disabled option is specified. **The source dropdown itself is specified on no screen in this set:** this page and `order-detail.md` `[L-4]` both treat its value as provenance already chosen at purchase time, so a stock gate on it cannot be written here without first inventing the control it would attach to.
  - **How it reaches an operator.** Where the adjustment took `Total` below `Reserved`, Inventory auto-comments **each holding order** and notifies `#fulfillment-admin-comments` `[G-7]` (`stock-status.md` `[DC-37]`), so it arrives on this page through the order's comment panel `[L-S1-19]` and the hub `[L-S1-3]`, like every other cross-screen exception. Correcting the stock figure itself is Inventory's — the quantities **this** page moves (inbound, cancel inbound, return restock) are movements, which is a different thing from a book correction.
...
Channels are cited with their ID on first mention where `_inputs/slack-routing.md` publishes one. Payload fields are verbatim from `_inputs/slack-routing.md` (CONFIRMED 2026-08-03).

| # | Trigger on this page | Channel | Payload | Mention target |
|---|---|---|---|---|
| 1 | **M2b** `Send to Missing Tracking List` `[L-M2b]` | `#unrecognized-tracking` (no channel ID is published in `_inputs/slack-routing.md`; it is resolved at wiring time) | tracking no., product, qty, memo, registrant, suspected orders | **@mention every suspected PIC** — once per person even when several of their orders match `[G-7]` (owner-decided 2026-08-03; `tracking-missing`'s push-not-poll design depends on it) |
...
| **OQ-2** — **RESOLVED, OWNER-DECIDED 2026-08-04** | Should the **Live Barcode Feed export** be restricted, approved, or merely logged as a data-egress event? **Answer: allowed without restriction, and logged in full.** Two reasons, both structural rather than convenience. (1) v1 ships a **single admin role** [G-15], so "authorised users only" and "approval required" have nothing to gate on — implementing either would make this button the only permission check in the entire system. (2) The feed is **already fully visible on screen** to every admin, so blocking the export does not withhold the data; it only adds friction while a screenshot achieves the same egress. The real control point is who may open the screen, and v1 has no such model. | The feed carries a full scan history including operator identity, so an export is personal-activity data leaving the system; no input document addressed export governance, and the spec declined to invent an approval flow because that would have invented an owner for it. Answered 2026-08-04 on the two structural grounds in the previous column. | **Export stays enabled; every export persists `[DC-47]` with actor, range, row count, format and delivery.** Two riders: `delivery = email` is restricted to **internal company domains** — it is the only path by which this data leaves the building unaided, and it is the one worth constraining — and this export is recorded as the **first gate candidate** if a permission model is introduced post-v1. |
...
| **CP-2** | **`ready-to-outbound` writes a third line status `OUTBOUNDED`**; neither this page nor Order Detail has one, and Order Detail's rendering contract is exhaustive. | Outbound is **order-level only**. The line machine is exhaustively `PENDING ↔ INBOUNDED` `[L-S1b-21]`, which is why `Cancel Outbound` touches no line state `[BR-11]`. | **Closed 2026-08-03 — this page won.** `ready-to-outbound` v1.2 dropped the line-status write: `BR-22` and `[DC-9]` now read "order-level, writes no line-level status", and its §10.1 reversal chain records the correction. **There is no `OUTBOUNDED` line status anywhere in the set** and the line machine stays exactly `PENDING ↔ INBOUNDED`. Nothing further is owed. *(This cell still read "RTO must drop the transition" until 2026-08-04 — stale text, not a live conflict.)* |
| **CP-3** ✅ **RESOLVED 2026-08-04** — published in `[G-5]`: **this page renders it, Order Detail does not**, and `ready-to-outbound` §1.4/§4.2 was corrected (it had named Order Detail). `[PD-80]` still governs the label form only. | **`OTHER (channel)` on order-facing rows** — `[G-5]` says the order-facing badge set is "exactly 4"; this page renders a fifth value, `ready-to-outbound` says Order Detail renders it, Order Detail says it never does. | **Declared here** as an explicit page delta on `[G-5]` with rationale `[L-S1-6]`: the label is a rendering pass-through of the route inherited from the Inbound Request `[BR-5]`, never a fifth selectable route. | `[G-5]` must state which surfaces may render `OTHER (channel)` on order-facing rows; then whichever of `ready-to-outbound` §1.4/§4.2 or `order-detail` `[L-4]` disagrees is corrected. `[PD-80 · OWNER-PENDING]`. |
...
Every decision that shaped this screen, 2026-07-09 → 2026-08-08, including reversals and removals. Sources: the 2026-07-09 and 2026-08-02 decision ledgers, the wireframe legends (dated in-place), `_review.md` adjudications C-1…C-12, the PD register, the owner's 2026-08-03 emphasis pass, and the owner's 2026-08-08 manual-stock-adjustment decision.
...
| **2026-08-08** | **Existing inventory meets Inventory's manual stock adjustment, stated in both directions.** Inventory gained a per-row restatement of a SKU's warehouse `Total`, down to `0` (`stock-status.md` `[L-19]`; `_global-rules.md` v1.6). *After* a pick: nothing on this page is undone or re-rendered — the line keeps the deducted route, the `row-exist` tint and its `Location`, because `Location` is a locator and not a quantity, and the adjustment is an `ADJUST` book correction rather than a movement, so **no `[DC-39]` is written and its `origin` enum gains no member**. *Before* a pick: a SKU at `0` has nothing to deduct, and this page **renders no stock quantity and performs no availability check**, so no gate, warning or disabled option is specified — the source dropdown is specified on no screen in this set, so a gate on it would have to invent the control it attaches to. The operator-facing path is the auto-comment Inventory posts on each order holding the affected units, arriving through `[L-S1-19]` and `[L-S1-3]`. **No id was minted, renumbered or retired**; no QA scenario changed | `[L-S1-6]` `[L-S1-17]` `[DC-39]`; `stock-status.md` §3.22 `[BR-3]` `[DC-37]` | active |
