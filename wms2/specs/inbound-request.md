...
3. **The Comments-hub pane strings on this wireframe diverged from the rest of the corpus — fixed 2026-08-03.** This page shipped `Comments where I'm tagged` / `Mark all as read` / `Comments I saved` / `Unstar to remove from list`. Of those four, two had a corpus majority (mentions header `Comments mentioning me · Click to open the order`, 5 pages; read-all `Mark all read`, 6 pages) and two did not — the saved header split 5 / 1 / 2, and the unstar hint split `… from list` 3 (this page, `order-management`, `view-orders`) / `… from the list` 2 / `… from this list` 2 / `Unstar to remove` 1. *(An earlier tally here read 2 / 2 / 2 / 1 because it omitted `view-orders`; the corrected count still shows no majority, which is why the hint was resolved on standard-English grounds rather than by vote.)* `[G-7]` v1.2 now publishes the set as byte-exact contract (HUB-1…HUB-7), and **`[IR-WFX-1]` was applied**: this wireframe, §3.1.13 and QA-F-02 / QA-F-03 all moved to the canonical strings in one commit. See §3.1.13 and QA-F-11.
...
**Pane headers — `[G-7]` canonical strings.** This page carries no wording of its own: the `@ Mentions` pane header reads `Comments mentioning me · Click to open the order` (`[G-7]` HUB-1) with the action `Mark all read` (HUB-4), and the `★ Saved` pane header reads `Saved comments · Click to open the order` (HUB-2) with the hint `Unstar to remove from the list` (HUB-3). Until 2026-08-03 the wireframe shipped `Comments where I'm tagged` / `Mark all as read` / `Comments I saved` / `Unstar to remove from list` and this spec deliberately asserted those, because `[G-7]` published no canonical to conform to and inventing one page-side would only have moved the fork. `[G-7]` v1.2 published HUB-1…HUB-7 and **`[IR-WFX-1]` was applied**; the wireframe and QA-F-02 / QA-F-03 moved with it in the same commit, and QA-F-11 keeps the `[ADMIN]` cross-page identity check. See §2.4 observation 3.

**Star toggle.** `★` toggles saved state per user (`DC-12`). `Mark all read` clears the unread badge (`DC-13`).
...
Then the `@ Mentions` pane header reads `Comments mentioning me · Click to open the order` with the action `Mark all read` — the `[G-7]` canonical HUB-1 / HUB-4 strings since the 2026-08-03 `[IR-WFX-1]` fix; QA-F-11 carries the `[ADMIN]` cross-page identity check
...
Then the pane header reads `Saved comments · Click to open the order` with the hint `Unstar to remove from the list` — the `[G-7]` canonical HUB-2 / HUB-3 strings since the 2026-08-03 `[IR-WFX-1]` fix; see §3.1.13 and QA-F-11.
...
**QA-F-11** `[ADMIN]` — covers `[L-S1-14]`, `[G-7]` HUB-1…HUB-7
Given the built admin's Comments hub on this page
Then its shared strings — mentions pane header, saved pane header, unstar hint, read-all action, search-results header, empty-search state and search placeholder — are **byte-identical to the `[G-7]` canonical set** (`Comments mentioning me · Click to open the order` · `Saved comments · Click to open the order` · `Unstar to remove from the list` · `Mark all read` · `{n} results · newest first · click to open the order` · `No matching comments` · `🔍 Search all comments — order no. · author · text`) and byte-identical to the same strings on the other seven screens
And this page invents no local wording for any of them; the pre-2026-08-03 strings `Comments where I'm tagged` / `Mark all as read` / `Comments I saved` / `Unstar to remove from list` appear nowhere
...
*Runnable since `[G-7]` v1.2 published the set (2026-08-03). The first four elements are also asserted at `[WF]` tier by QA-F-02 / QA-F-03 and now agree with this scenario; the last three stay `[ADMIN]`-only because this wireframe builds no search pane.*
