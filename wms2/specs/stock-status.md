...
Version 1.2 · 2026-08-04 · Page slug `stock-status` · Screen name **Inventory** (renamed from "Stock Status" 2026-07-22).

> **Amendment 2026-08-04 (owner) — money is removed from the Stock Audit and from this page entirely.** The audit reports **unit differences only** (`−1`, `+2`); there is no `Loss` column, no loss total, no product-cost lookup, and no monetary field on any event. See `[BR-4]`, `[BR-31]`, §9.1 #16 and the three 2026-08-04 rows in §10 for the full inventory of what was deleted and what replaced it. **The wireframe and this spec were changed in the same round and agree** — `wms2/stock-status/index.html` now renders the count-based strings asserted below, and carries inline "do not re-add" comments at each removal site. No `[INV-WFX]` entry is owed: this is a closed change, not a declared divergence. Money belonging to other screens (Inbound Request `Unit Cost` / `JIT Price`, Order Detail order amounts) is out of this amendment's scope and unchanged.
...
- **One line at the bottom.** The audit summary is a single bottom bar carrying the session's **difference counts** rather than a per-row rollup, because at arm's length on a cart only one line is readable at a glance.
- **The audit counts units, never money.** A count difference is reported as `−1` / `+2` — how many units are missing or extra — and nothing on any audit surface converts it to a value. Product cost is **not read, not displayed, and not stored** by the audit: no `Loss` column, no loss total, no cost lookup, no money field on any audit event. Owner decision **2026-08-04** (§10); it retires `[BR-4]`'s money design, `[BR-29]`, `[BR-31]`, `[E-25]`, `[E-61]`, `[E-72]` and §9.2 `OQ-1`. Money that is **not** the audit's — the Inbound Request's `Unit Cost` / `JIT Price` and Order Detail's order amounts — is untouched by this decision and lives on those pages.
...
Opens **Past Audit Logs** (`[L-15]`), reads one row per month, and tracks the **adjustment count** toward **0**. June 2026 (5 adjustments, −4 / +1) is the outlier and is under root-cause investigation (suspected picking mis-outbound) — that row is why the per-session adjustment breakdown is on the list at all. *(Before 2026-08-04 this row was read as a money figure, `−₩128,460`; the loss column was removed by owner decision and the same session is now read by its ± counts — §10.)*
...
`SKU` · `Image` · `Product Name` · `Product Name KR` · `Size` · `Barcode` · `Sourcing Route` · `Location` · `Total` · `Reserved` · `Available` — plus **two** audit-only columns `Counted Qty` · `Diff` that are hidden outside audit mode `[G-14]`.

> **A third audit column `Loss (₩)` must NOT exist.** It was removed by owner decision 2026-08-04 (§10, §9.1 #16): the audit reports unit differences, never their value. The wireframe was updated in the same round and no longer draws it; it carries an inline comment at the removal site instead. Re-adding the column, or any per-row money cell, is a defect.
...
4. The two `.audcol` columns (`Counted Qty`, `Diff`) become visible in header and every body row `[G-14]`. There is no third audit column — see §3.1.
...
- **`Diff` is the whole of the per-row output.** No third cell is computed from it. `Loss (₩) = Diff × product cost` was the rule here until **2026-08-04**, when the owner removed money from the audit; the column, the cost lookup, and the "a difference is only meaningful in money" rationale behind it are all retired (`[BR-4]`, §10). A row's difference is complete when the signed count renders — nothing downstream needs a cost, so no SKU is ever "missing" one `[E-25 · retired]`.
...
> **Wireframe fixture note (not a defect).** The shipped wireframe pre-seeds **two** rows with a non-matching count so the `Diff` column has something to demonstrate: SKU `100005104` shows `Counted Qty 17` against `Total 18`, and SKU `100012534` shows `Counted Qty 11` against `Total 9`. The other nine rows are prefilled equal to `Total`. QA asserts this census exactly (`QA-AUD-04`) and must not report it as a prefill bug.

**`#auditSummary` bar.** Text: `Adjustments: {n} ({−a} / {+b}) · New additions: {m}`, followed by the button `Confirm Audit Differences (ADJUST log)`. `{n}` counts rows with `Diff ≠ 0`; `{−a}` / `{+b}` are the negative and positive halves of that count, in the same `{count} ({negative} / {positive})` form `[L-15]` already uses; `{m}` counts `[L-13]` new additions, which are reported separately and never folded into `{n}` `[BR-35]`. It recomputes on every committed count. Wireframe fixture value: `Adjustments: 2 (−1 / +2) · New additions: 1`.

> **No money in this bar.** Until 2026-08-04 it read `Total stock loss (sum of diff × product cost): {total} — target 0` and carried the centre manager's money KPI. The owner removed money from the audit; the bar now carries counts and **must not** regain a currency value, a cost-weighted total, or a monetary target (§10, §9.1 #16). The wireframe now renders the count form; `QA-AUD-03` asserts it at `[WF]` tier.
...
- On Add, a new row is inserted **at the top of the table body**, tinted purple, with: SKU, Korean name + a purple `[NEW]` marker, size, `Total 0`, `Reserved 0`, `Available 0`, `Counted Qty = entered qty`, `Diff = +qty` green. There is no third cell — the retired `Loss (₩)` cell that rendered `— (신규)` went with the column on 2026-08-04 (§3.1, §10). Removing it also removes the page's only Korean **non-product** string; see the allow-list note in §8.0.
...
**Commit.** New rows merge into `[L-M1]` as `ADJUST(+qty)` and persist as `[DC-9]` (`audit.new_item_added`) with `system 0 → counted n`, location, and a NEW-ADDITION flag. New additions are **counted separately from the adjustment count** — they are reported in their own `New additions` figure, never folded into `Adjustments` — and are **included in `SKUs Checked`** `[BR-35]` `[E-81]`. *(Until 2026-08-04 this separation was expressed as "excluded from the total loss"; with money gone it is a counting rule, and the same rows are still kept out of the adjustment tally.)*
...
**Table columns, exactly:** `Audit Date` · `Auditor` · `SKUs Checked` · `Adjustments` · `New Additions` · `Detail`. **Six columns — a seventh `Total Loss` column must NOT exist** (removed 2026-08-04, §9.1 #16).

Wireframe rows (fixtures, byte-accurate for QA):

| Audit Date | Auditor | SKUs Checked | Adjustments | New Additions | Detail |
|---|---|---|---|---|---|
| 2026-07-22 | Yongwon | 10 | 2 (−1 / +2) | 1 | `View ADJUST events` → `[L-M2]` |
| 2026-06-30 | Dean | 9 | 5 (−4 / +1) | 0 | `View ADJUST events` → `[L-M2b]` |
| 2026-05-31 | Dean | 9 | 0 | 0 | `—` (no link) |
...
- `Adjustments` renders as `{count} ({negative} / {positive})`. It is the column the centre manager reads: **the goal is to bring the adjustment count to 0.** Colour: **a zero count renders green**, every non-zero count renders in the table's default colour (`QA-LOG-09`). The retired `Total Loss` column's three-tier amber/red/green scale was **not** transferred to this column — only the green "no adjustments" marker survived, and no severity scale on the count is specified or implied.
- **`Total Loss` — RETIRED 2026-08-04.** It rendered `+₩46,260` / `−₩128,460` / `₩0 · target met` with amber / red / green colouring. The owner removed money from the audit, so the column, its colouring rule, and the ` · target met` suffix are all gone. The month-over-month signal it carried is now read from `Adjustments`. The wireframe was updated in the same round and draws six columns.
- A session with zero adjustments still gets a row and renders `—` in Detail (no modal to open) `[E-24]`.
- Note, verbatim: `The center manager's goal is to bring the adjustment count to 0. Use "View ADJUST events" for each session's adjustment detail — individual events are also recorded as ADJUST type in each SKU's Stock History.` *(First sentence rewritten 2026-08-04: it previously named the total loss. Second sentence unchanged.)*
...
- Two tabs: `@ Mentions` (with the unread count) and `★ Saved`. `@ Mentions` pane header: `Comments mentioning me · Click to open the order`, with `Mark all read` on the right. `★ Saved` pane header: `Saved comments · Click to open the order`, with `Unstar to remove from the list` (`[G-7]` HUB-3).
...
- **Cross-page copy variance — resolved 2026-08-03.** `[G-7]` states the hub is identical on all eight screens, and as of `[G-7]` v1.2 it publishes the strings as byte-exact contract (HUB-1…HUB-7). This page already matched the majority on six of the seven; the seventh — the unstar hint — read `Unstar to remove from this list` because that is what the shipped wireframe rendered. The corpus had no majority on that string (`… from list` 3 / `… from the list` 2 / `… from this list` 2 / `Unstar to remove` 1), so it was resolved to the standard English form `Unstar to remove from the list` (HUB-3), which three specs had already named as the intended canonical. The wireframe, this section and `QA-COM-*` moved together in one commit — the divergence is closed, not relocated.
...
**Table columns, exactly:** `SKU` · `Product` · `Location` · `System` · `Counted` · `Diff` · `Action`. **Seven columns — an eighth `Loss` column must NOT exist** (removed 2026-08-04, §9.1 #16).
...
- New-addition rows are tinted purple, carry a purple `[NEW]` marker, show `System 0` → `Counted n`, and `Action` = `ADJUST(+n) added`. The retired `Loss` cell that rendered `— (신규)` is gone with the column.
...
**Summary note, verbatim:** `Adjustments: {n} ({−a} / {+b}) · New additions: {m} (counted separately) — on confirm, Current Stocks · Available update immediately; monthly audit logs retained.`

> **Money removed 2026-08-04.** This note previously read `Total stock loss: {total} (target 0) · the {n} new addition(s) are not losses — …`. The owner removed money from the audit, so the money total and the `(target 0)` qualifier are retired and the note now leads with the same counts the `#auditSummary` bar carries (§3.3). The `{m}`-templatisation discipline that `[INV-WFX-1]` established on 2026-08-03 is preserved and now applies to `{n}`, `{−a}`, `{+b}`, `{m}` alike: **every one is computed from the modal's actual rows and is never a literal** (`QA-AUD-44`, `[ADMIN]`). `QA-AUD-23` asserts the count form at `[WF]` tier.
...
1. One `ADJUST` movement per row → `[DC-8]` (`audit.adjust_confirmed`) with `system qty → counted qty`, diff, and location at count time; each also materializes as an `ADJUST` row in that SKU's Stock History `[L-8]`. **No monetary field and no product-cost value is written** (2026-08-04).
2. Each new addition → `[DC-9]`.
3. The session closes → `[DC-10]` (`audit.session_confirmed`) with audit date, auditor, SKUs checked, adjustment counts (± breakdown), new additions, confirm timestamp. **No loss total** (2026-08-04). This is the `[L-15]` row.
...
6. Green toast: `Stock audit confirmed — {N} ADJUST events recorded`. *(The trailing ` · total loss {total}` clause was removed 2026-08-04.)*
...
**Recompute at confirm.** System quantities are re-read at confirm time — **quantities only; there is no cost to re-read** (2026-08-04, `[E-61 · retired]`). If stock moved during the walk, `Diff` is recomputed against the **current** system quantity and the modal re-renders with a non-green toast `Stock changed during the audit — differences recalculated` before Confirm can proceed `[PD-6 · OWNER-PENDING]` `[E-22]`. A scope row deleted or merged system-wide mid-session is dropped from the batch and named in the same toast `[E-82]`.
...
**Columns, exactly:** `Time` · `SKU` · `Product` · `Location` · `System → Counted` · `Adjustment`. **Six columns — a seventh `Loss` column must NOT exist** (removed 2026-08-04, §9.1 #16).
...
- `Adjustment` renders as an `ADJUST(±n)` badge. New additions are tinted purple and carry `[NEW ADDITION]`. The retired `Loss` cell that rendered `—` on those rows is gone with the column.
- `[L-M2]` note, verbatim: `Each event is also recorded as ADJUST type in that SKU's Stock History — search the SKU and filter by type to inspect individually. Adjustments: 2 (−1 / +2) · New additions: 1 (counted separately).` *(Second sentence rewritten 2026-08-04 — it previously read `Total loss +₩46,260 (new additions excluded).`)*
- `[L-M2b]` note, verbatim: `Adjustments: 5 (−4 / +1) — June is the outlier; root-cause investigation in progress (suspected picking mis-outbound).` *(Rewritten 2026-08-04 — it previously read `Total loss −₩128,460 — June exceeded the loss target; …`. The session it describes and the investigation it points at are unchanged; only the money framing is gone.)*
- The wireframe was updated in the same round: both notes render the count form above and the `Loss` column is gone. `QA-LOG-04` / `QA-LOG-06` assert the two notes at `[WF]` tier.
...
- **Shipped wireframe string (stale — `[INV-WFX-2 · proposed]`):** its final sentence still reads `For order-linked inbound, use Request Inbound on View Orders / the order detail.` **`Request Inbound` is a retired control name.** Neither named page has such a button — View Orders uses row `Inbound` / `Inbound + Outbound` / `Bulk Inbound (Selected)`, Order Detail uses per-row `Inbound` + `Bulk Inbound Selected Items` — and Order Detail's spec forbids the name reappearing anywhere. Promoting the stale sentence to a byte-exact production contract would send an operator to a button that does not exist. **APPLIED 2026-08-03** — the wireframe note now reads the corrected copy (`… use the row Inbound buttons on View Orders or Order Detail.`); `QA-FRM-03` asserts the corrected string at `[WF]` tier, `QA-FRM-20` additionally asserts the total absence of `Request Inbound` at `[ADMIN]` tier.
...
- Whether the **two** audit-only columns (`Counted Qty`, `Diff` — the third was removed 2026-08-04, `[BR-5]`) are included when audit mode is active is a **developer decision**; the recommended default is to include them with the session id in the filename `[E-54]` (§9.2). **An export never carries a money column**, in audit mode or out of it `[BR-31]`.
...
| **BR-4** | **RETIRED 2026-08-04 — replaced in place, ID kept so the slot is not a hole.** The retired rule was *"Loss = Diff × product cost; the session total targets 0 and is the centre manager's KPI"*, with the cost source deferred to dev. **The rule now is: the audit reports unit counts and nothing else.** A difference is a signed integer (`−1`, `+2`); the session summary is `Adjustments: {n} ({−a} / {+b})` with `New additions: {m}` reported separately; the KPI the centre manager tracks toward 0 is the **adjustment count**. No product cost is read, rendered, or stored by any audit surface. | Owner, 2026-08-04: *"재고 실사의 손실액은 계산하지 말자. 그냥 −1 +1 이런 것만 보이게 하자."* The count is what the auditor can verify at the shelf; the money was an unbacked derivation on top of it (its cost basis was never even decided — `OQ-1`, now void). New stock found still does not belong in the difference tally, which is why `New additions` stays a separate figure. | Money design 2026-07-22 → **retired 2026-08-04** |
| **BR-5** | **Page delta on `[G-14]`:** on this page the audit-mode-only set is exactly `Counted Qty`, `Diff`, `#auditSummary`, and `.audrow` — **four members, not five**; the third audit column `Loss (₩)` was removed 2026-08-04 `[BR-4]`. Nothing else is gated on audit mode. `[G-14]` | The daily user of this screen is not an auditor. Permanently visible count boxes invite accidental edits and make the table unreadable at cart distance. | 2026-08-03 · membership reduced 2026-08-04 |
...
| **BR-28** | **Page delta on `[G-15]`: this page adds no gate of its own.** The **two** actions an implementer would most plausibly gate — audit start/confirm `[L-7]` `[L-M1]` and reservation release `[L-M4]` — are **ungated in v1**. `[G-15]` `[PD-1 · OWNER-PENDING]` *(A third candidate, "product-cost / `Loss (₩)` visibility", was listed here until 2026-08-04; there is no cost surface left to gate — `[BR-4]`, `[BR-29]`.)* | Six screens independently raised the same question; inventing per-page gates would produce eight inconsistent models. | Provisional 2026-08-03 · third item dropped 2026-08-04 |
| **BR-29** | **RETIRED 2026-08-04.** The retired rule was *"Loss (₩) and product cost are visible to whoever runs the audit — no admin-only split in v1"* `[PD-43 · OWNER-PENDING]`. Money was removed from the audit entirely `[BR-4]`, so there is nothing whose visibility could be split by role and the question the rule answered no longer exists. **This ID must not be reused to re-introduce a cost surface under a permissions argument.** | The rule existed only to keep a money KPI on the floor. With no money KPI, it is moot rather than reversed. | Provisional 2026-08-03 → **retired 2026-08-04** |
...
| **BR-31** | **REPLACED 2026-08-04 — from a formatting rule to a negative one. This page renders no monetary value at all.** The retired rule was *"All money on this page is KRW (`₩`), thousands-separated and signed … No `$`, no `USD`, no currency selector"*, which existed because the audit carried a loss KPI. With money removed from the audit `[BR-4]` there is no money left on Inventory in any pane or modal: **`₩`, `$`, `USD`, and any other currency mark must match nothing anywhere on this screen** (`QA-GLB-11`). Money that belongs to other screens — the Inbound Request's `Unit Cost` / `JIT Price`, Order Detail's order amounts — is unaffected and is not rendered here. | Keeping the ID as a negative preserves the original protective intent: it stopped a "currency fix" then, and it now stops a "put the value back" re-implementation. Deleting the row would leave nothing to cite. | KRW rule 2026-07-22 / 2026-08-03 → **replaced by the no-money rule 2026-08-04** |
...
| **DC-6** | `stock.adjust_loss` | `[L-M4]` restock = **No**, or the auto-remainder of an under-restock | operator | SKU | `ADJUST(−n)`; classification `damaged / lost`; source `m4-writeoff` / `m4-remainder`; memo id? — **units only, no monetary amount** (the `loss amount ₩` field was removed 2026-08-04: with `OQ-1` void there is no specified cost basis on this page to value it with, and `[BR-31]` now forbids money here. The event name and every other field are unchanged) | Stock History `ADJUST` row |
...
| **DC-8** | `audit.adjust_confirmed` (one per SKU with `Diff ≠ 0`) | `Confirm — record N ADJUST events` `[L-M1]` | auditor | SKU + session id | `system qty → counted qty`; diff `±n`; location at count time — **no money field** (`loss = diff × product cost` and *"the cost value actually applied"* were removed 2026-08-04, `[BR-4]`; the diff **is** the record) | `[L-M2]` row **and** the same SKU's Stock History `ADJUST` row (one event, two views) |
| **DC-9** | `audit.new_item_added` | `[L-13]` Add, committed at `[L-M1]` Confirm | auditor | SKU + session id | `system 0 → counted n`; location; `NEW ADDITION` flag; **excluded from the adjustment count and reported as `New additions`** (was "loss excluded from the session total" until 2026-08-04); counted toward `SKUs Checked` `[BR-35]` | `[L-M2]` purple row; SKU's Stock History `ADJUST(+n)` |
| **DC-10** | `audit.session_confirmed` | `[L-M1]` Confirm | auditor | audit session | audit date (= session **start** date `[BR-36]`); auditor; SKUs checked; adjustments count with ± breakdown; new-additions count; confirm ts; the acknowledgement flag from `[DC-25]` if the gate fired — **no `total loss ₩`** (removed 2026-08-04, `[BR-4]`) | `[L-15]` row |
...
Channels are CONFIRMED 2026-08-03 per `_inputs/slack-routing.md`. Payload fields are verbatim from that file.
...
- **Procurement Hub is excluded from this program entirely** (owner decision 2026-08-02: no sweep, no English conversion, no spec). **Inventory now has no dependency on it at all.** The single touchpoint used to be a costing **read** — FIFO lot cost feeding the audit `Loss (₩)` calculation — and it died with the loss column on 2026-08-04 `[BR-4]`, `OQ-1` void. There is no costing lookup, no FIFO COGS read, and nothing else for an implementer to wire.
...
| `[E-54]` | **Export while audit mode is on** | The **two** audit columns `Counted Qty` and `Diff` are included by default (the third, `Loss (₩)`, was removed 2026-08-04 — an export must not reintroduce it `[BR-31]`) and the session id is carried in the filename. Persists `[DC-19]` with `audit-mode = true`. Final column policy is a dev decision (§9.2) |
...
| `[E-72]` | **Money rendering** — **RETIRED 2026-08-04, replaced by a negative** | There is **no money on this page** to render. The retired entry specified `₩` with thousands separators and an explicit sign (`+₩61,260`, `−₩15,000`, `₩0`, `— (신규)`); money was removed from the audit and the ID is kept as the negative assertion instead: **no currency mark and no monetary value appears in any pane or modal** `[BR-31]` `[BR-4]`. Signed **counts** (`−1`, `+2`) are not money and are unaffected |
...
| `[E-24]` | **Audit with zero differences** | Confirm is allowed. The session is recorded with 0 adjustments and 0 new additions, and appears in `[L-15]` with `Adjustments 0`, `New Additions 0`, and `—` in the Detail column (no ADJUST modal exists). Precedent: the 2026-05-31 session. *(The retired `₩0 · target met` rendering went with the `Total Loss` column, 2026-08-04.)* |
| `[E-25]` | **SKU has no product cost** — **RETIRED 2026-08-04** | **Not an edge case any more: the audit never reads a product cost**, so "no cost" cannot arise `[BR-4]`. The retired behavior was *"`Loss (₩)` renders `—`, the SKU still produces an `ADJUST` event, and it is excluded from the total-loss arithmetic"*. The surviving half of that — **every SKU with `Diff ≠ 0` produces an `ADJUST` event regardless of what else is known about it** — is unconditional and is asserted by `QA-AUD-25`. ID kept so nothing reintroduces a cost-availability branch |
...
| `[E-61]` | **Product cost changes between count entry and confirm** — **RETIRED 2026-08-04** | Moot: no cost is read at any point in the audit `[BR-4]`, so a cost change cannot affect a session. The retired behavior pinned the confirm-time cost onto `[DC-8]` for reproducibility. What still holds — and is specified at `[E-22]`, not here — is that **system quantities** are re-read at confirm. ID kept so a cost re-read is not reintroduced under this number |
...
| `[E-79]` | **A session whose only non-zero rows are `[L-13]` new additions** | Confirm is allowed. `{N}` in the button counts the new additions; the **adjustment count is 0** because new additions are counted separately `[BR-4]` `[BR-35]`; `[L-15]` shows `Adjustments 0`, `New Additions N`. *(The retired `Total Loss ₩0 · target met` cell went with the column, 2026-08-04.)* |
...
**Money removal (2026-08-04) — how to read the rows it touched.** The owner removed all money from Stock Audit. **The wireframe and this section were changed together in the same round and agree**, so every `[WF]` row below is runnable today against the shipped file — there is no pending drawing update and no `[INV-WFX]` entry outstanding. Two markers appear below:

- **RETIRED 2026-08-04** — the scenario asserted a fact that no longer exists in the contract at any tier (a product-cost precondition that can no longer arise). It is not runnable and will not become runnable. The ID is kept so numbering stays stable and so nobody re-adds the assertion under a new number.
- **rewritten 2026-08-04** — the scenario kept its ID and its job but now asserts the money-free string, column count, or absence. It runs as written.

An `[ADMIN]`-tier row touched by the removal is simply rewritten and carries no marker. A `[WF]` row whose only change was dropping a money clause likewise carries no marker.
...
**Korean allow-list (for `QA-GLB-09`).** Every Hangul run on this page — all four panes and all six modals — must be a substring of one of these **16 product-name strings** and nothing else:
...
> **17 → 16, 2026-08-04.** The seventeenth entry was the data string `신규`, which existed **only** as `— (신규)` inside the retired `Loss (₩)` / `Loss` cells (`[L-5]` audit row, `[L-13]` inserted row, `[L-M1]`). Removing money removed the page's last non-product Hangul, so the contract allow-list is now exactly the product names. The wireframe dropped both `— (신규)` cells in the same round, so the shipped file already reads 16 and `QA-GLB-09` is runnable as written.
...
**Wireframe demo limitations — QA must tag these `[ADMIN]`, never file them as bugs.** The Current Stocks search box, the Location and Sourcing Route selects, the `.filterchip` buttons, the column headers, both Export buttons, and the `🔍 Search` button are **inert**. Typing into a `.qty-in` does **not** recompute `Diff` or `#auditSummary` — those are static fixtures. `[L-M4]` `Confirm` only closes the modal. No toasts, no sounds, and no pagination controls exist anywhere. Any `Cancel Inbound` button opens the same Order-409112 fixture modal `[E-59]`.

**Totals: 200 scenario IDs — 75 [WF] · 125 [ADMIN] · 85 negative/boundary tests (42.5%).**
Per block: NAV 12 · CS 18 · LOC 18 · AUD 44 · LOG 9 · RES 26 · HIS 18 · FRM 20 · COM 15 · EXP 6 · GLB 14.

**2026-08-04 status against those totals (IDs unchanged — never renumber, and no ID was added):**

- **2 RETIRED**, both [ADMIN] — `QA-AUD-32` and `QA-AUD-33`, whose product-cost preconditions can no longer arise. → **198 live · 75 [WF] · 123 [ADMIN], all runnable.**
- **19 rewritten in place (13 [WF] + 6 [ADMIN]), ID and tier unchanged.** [WF]: `QA-CS-14`, `QA-AUD-01`, `QA-AUD-03`, `QA-AUD-06`, `QA-AUD-16`, `QA-AUD-23`, `QA-LOG-02`, `QA-LOG-03`, `QA-LOG-04`, `QA-LOG-06`, `QA-LOG-09`, `QA-GLB-09`, `QA-GLB-11`. [ADMIN]: `QA-AUD-09`, `QA-AUD-25`, `QA-AUD-31`, `QA-AUD-38`, `QA-AUD-44`, `QA-EXP-03`. Each asserts the money-free string, column count, colour, or absence that the wireframe now renders.
- **`QA-LOG-09` was rewritten, not retired.** Its old subject (the `Total Loss` colour scale, ` · target met`) is gone, but the surviving colour rule from §3.11 — a zero adjustment count renders green — moved onto the `Adjustments` column under the same ID. It is asserted there and **not** in `QA-LOG-02`, so a colour regression names exactly one scenario.
- **Negative/boundary count unchanged at 85** (84 `NEGATIVE` + 1 `BOUNDARY`, over the 200-ID space) — neither retired row was one, and `QA-GLB-11` stays negative (it flipped from a format check to an absence check).
- **The runner agrees.** `_verify/prehandoff/qa-stock-status.py` executes **75** scenarios — exactly this section's `[WF]` set — and passes 75/75 against the shipped wireframe.
- **No ID was added, removed, or renumbered.**
...
| **QA-CS-14** | [WF] | **Given** `#p-current` and preflight step 2 done, **Then** reading the header row with `innerText` yields exactly `SKU, Image, Product Name, Product Name KR, Size, Barcode, Sourcing Route, Location, Total, Reserved, Available` for the visible headers, plus **exactly two** `th.audcol` headers `Counted Qty`, `Diff` whose computed `display` is `none` — **and no third `th.audcol`**, the retired `Loss (₩)` header being a defect if present. *(Reading with `textContent` instead yields `Location12` and `Available11` — that is the annotation dot, not a defect. See §2.4.)* `[L-5]` `[G-14]` `[BR-5]` |
...
| **QA-AUD-03** | [WF] **rewritten 2026-08-04** | **Given** audit mode is on, **Then** `#auditSummary` normalised `innerText` reads exactly `Adjustments: 2 (−1 / +2) · New additions: 1` before the button, it contains a button whose `innerText` is exactly `Confirm Audit Differences (ADJUST log)`, and **no forbidden money token** — `QA-GLB-11`'s canonical set — appears inside `#auditSummary`. The two counted rows are the `QA-AUD-04` fixture pair; the one addition is the `[NEW]` row `QA-AUD-22` sees. *(Until 2026-08-04 this row asserted the money string `Total stock loss (sum of diff × product cost): +₩46,260 — target 0`; money was removed from the audit `[BR-4]`, so the assertion was replaced, not deleted — the button-label clause is unchanged.)* `[L-7]` `[BR-4]` `[BR-31]` |
...
| **QA-AUD-06** | [WF] **rewritten 2026-08-04** | **Given** audit mode is on, **Then** the `Diff` cells read `−1` on SKU `100005104`, `+2` on SKU `100012534`, and `0` on the other nine rows; the two non-zero rows reconcile with `#auditSummary`'s `Adjustments: 2 (−1 / +2)` `QA-AUD-03`; **and each data row exposes exactly two `td.audcol`**, so no per-row money cell exists. *(Until 2026-08-04 this row also asserted `Loss (₩)` cells `−₩15,000` / `+₩61,260` / `₩0` and the arithmetic `−15,000 + 61,260 = +46,260`; the column was removed `[BR-4]`, and the count reconciliation replaces the money reconciliation.)* `[L-7]` `[BR-4]` `[BR-5]` |
...
| **QA-AUD-09** | [ADMIN] | **Given** audit mode and the row for SKU `100005104` (`Total 18`), **When** the operator types `17` and presses `Tab`, **Then** `Diff` renders `−1` in red, **no money cell is rendered on the row** (2026-08-04, `[BR-4]`), `#auditSummary` recomputes its counts, focus moves to the **next row's** `Counted Qty` in walking order, and `[DC-24]` persists `prefill 18 → entered 17`. `[L-7]` |
...
| **QA-AUD-23** | [WF] | **Given** `#m-adjust` is open, **Then** the amber note begins `⚠ Reserved shortage check` and ends `None in this audit.`, and the second note reads exactly `Adjustments: 2 (−1 / +2) · New additions: 1 (counted separately) — on confirm, Current Stocks · Available update immediately; monthly audit logs retained.` *(Rewritten 2026-08-04: the note previously read `Total stock loss: +₩46,260 (target 0) · the 1 new addition is not a loss — …`. `[INV-WFX-1]`'s 2026-08-03 correction — the count is the session's real one, never a literal — is preserved and now covers all four numbers.)* The production contract stays the templatised form in §3.13, asserted at `[ADMIN]` tier by `QA-AUD-44`. `[L-M1]` `[BR-4]` |
...
| **QA-AUD-31** | [ADMIN] | **Given** an audit where every `Diff` is 0, **When** Confirm is pressed, **Then** the session is recorded with `0` adjustments and `0` new additions and appears in `[L-15]` with `Adjustments 0`, `New Additions 0`, and `—` in the Detail column. *(The retired `₩0 · target met` cell went with the `Total Loss` column, 2026-08-04.)* `[E-24]` |
| **QA-AUD-32** | [ADMIN] **RETIRED 2026-08-04** | Asserted that a SKU with no product cost renders `Loss (₩)` as `—` while still producing an ADJUST event, and is excluded from the session total. The audit no longer reads a product cost `[BR-4]` `[E-25 · retired]`, so the precondition cannot occur. **The clause worth keeping was never cost-specific and is already covered:** every row with `Diff ≠ 0` produces a `[DC-8]` ADJUST event — `QA-AUD-25`. Do not re-add a cost-availability branch under this ID. |
| **QA-AUD-33** | [ADMIN] **RETIRED 2026-08-04** | Asserted that the confirm-time product cost is the one applied and is stored on `[DC-8]` for reproducibility. No cost is read or stored anywhere in the audit `[BR-4]` `[E-61 · retired]`. **The confirm-time re-read that does still happen is of system quantities**, asserted by `QA-AUD-30`. Do not re-add a cost re-read under this ID. |
...
| **QA-AUD-38** | [ADMIN] | **Given** a session whose only non-zero rows are `[L-13]` new additions, **When** Confirm is pressed, **Then** it succeeds, the **adjustment count is `0`** (additions are counted separately, not netted in), one `audit.new_item_added` `[DC-9]` event persists per added row (each carrying `system 0 → counted n`, the location, and the NEW-ADDITION flag), and `[L-15]` shows `Adjustments 0` / `New Additions {n}` — with **no money cell on the row** (2026-08-04). `[E-79]` `[DC-9]` `[BR-4]` |
...
| **QA-AUD-44** | [ADMIN] | **Given** the production admin with a session holding two adjustment rows (`−1`, `+2`) and **one** `[L-13]` new addition, **When** `[L-M1]` is opened, **Then** its summary note reads exactly `Adjustments: 2 (−1 / +2) · New additions: 1 (counted separately) — on confirm, Current Stocks · Available update immediately; monthly audit logs retained.` — **every number is templatised from the modal's actual rows and none is a literal.** Re-run with five adjustments (−4 / +1) and three additions and the same note reads `Adjustments: 5 (−4 / +1) · New additions: 3 (counted separately) — …`. **And** no forbidden money token — `QA-GLB-11`'s canonical set — appears in the modal. *(Rewritten 2026-08-04 — the note previously carried `Total stock loss: {total} (target 0)`; `[INV-WFX-1]`'s templatisation discipline from 2026-08-03 is preserved and extended to all four numbers. `QA-AUD-23` asserts the same string at `[WF]` tier against the shipped file.)* `[L-M1]` `[BR-4]` `[BR-31]` §3.13 |
...
| **QA-LOG-02** | [WF] **rewritten 2026-08-04** | **Given** `#m-auditlog` is open, **Then** the header row reads `Audit Date, Auditor, SKUs Checked, Adjustments, New Additions, Detail` — **six columns, no `Total Loss`** — and exactly three body rows exist: `2026-07-22 / Yongwon / 10 / 2 (−1 / +2) / 1`, `2026-06-30 / Dean / 9 / 5 (−4 / +1) / 0`, `2026-05-31 / Dean / 9 / 0 / 0`. **And** every body row has exactly **six** cells — no seventh survives anywhere — and no forbidden money token (`QA-GLB-11`'s canonical set) appears in any header or body cell. *(The `Total Loss` column and its three money cells `+₩46,260` / `−₩128,460` / `₩0 · target met` were removed 2026-08-04 `[BR-4]`. The `Adjustments` **colour** rule is deliberately **not** asserted here — it belongs to `QA-LOG-09`, which was rewritten onto that column rather than retired, so the colour lives in exactly one scenario.)* `[L-15]` `[BR-4]` `[BR-31]` §3.11 |
| **QA-LOG-03** | [WF] | **Given** `#m-auditlog` is open, **When** the agent clicks `View ADJUST events` in the `2026-07-22` row, **Then** `#m-adjlog` gains class `open` and its header's **normalised** `innerText` (§8.0 step 3b — the raw value appends `✕` from the close button) is exactly `2026-07-22 Stock Audit — 3 ADJUST events (Auditor: Yongwon, confirmed 14:20)`, and three body rows all timestamped `14:20:11`, the third tinted purple and carrying `[NEW ADDITION]`. *(The trailing `Loss` `—` cell on that row went with the column, 2026-08-04.)* `[L-M2]` |
| **QA-LOG-04** | [WF] | **Given** `#m-auditlog` is open, **When** the agent clicks `View ADJUST events` in the `2026-06-30` row, **Then** `#m-adjlog6` opens and its header's **normalised** `innerText` (§8.0 step 3b) is exactly `2026-06-30 Stock Audit — 5 ADJUST events (Auditor: Dean, confirmed 17:05)`, five body rows all timestamped `17:05:42`, and the note `Adjustments: 5 (−4 / +1) — June is the outlier; root-cause investigation in progress (suspected picking mis-outbound).` *(Rewritten 2026-08-04 — the note previously read `Total loss −₩128,460 — June exceeded the loss target; …`.)* `[L-M2b]` `[BR-4]` |
...
| **QA-LOG-06** | [WF] | **Given** `#m-adjlog` is open, **Then** its note reads exactly `Each event is also recorded as ADJUST type in that SKU's Stock History — search the SKU and filter by type to inspect individually. Adjustments: 2 (−1 / +2) · New additions: 1 (counted separately).` and every `Product` cell renders a Korean name whose first element child is a `<b>` brand. *(Second sentence rewritten 2026-08-04 — it previously read `Total loss +₩46,260 (new additions excluded).`; the first sentence and the brand-bold clause are unchanged.)* `[G-6]` `[G-8]` `[BR-4]` |
...
| **QA-LOG-09** | [WF] **rewritten 2026-08-04** | **Given** `#m-auditlog` is open, **Then** the `Adjustments` cell of the `2026-05-31` row — the zero-adjustment session — computes `color` `rgb(25, 135, 84)` (`--green`) per the §8.0 colour-token table, while the `2026-07-22` and `2026-06-30` rows' `Adjustments` cells carry the table's default colour; **and** no cell in the modal ends with ` · target met`. Structural fallback if the token drifts, asserted **in addition to** the token comparison and never instead of it: exactly one of the three `Adjustments` cells differs from the other two. *(Until 2026-08-04 this row asserted an amber / red / green scale on the `Total Loss` cell plus the ` · target met` suffix. That column was removed `[BR-4]`; the only colour that survived the change is the green "no adjustments" marker, which is what this ID now pins — and it is pinned **here and nowhere else**, so a colour regression names exactly one scenario. Do not re-add a money column or a money colour scale under this ID.)* `[L-15]` `[BR-31]` §3.11 |
...
| **QA-FRM-03** | [WF] | **Given** `#p-inbound`, **Then** the `.form-note` reads exactly `Record a warehouse inbound directly, without a specific order (return restock · manual inbound). Location auto-applies the SKU's single registered location. For order-linked inbound, use the row Inbound buttons on View Orders or Order Detail.` *(`[INV-WFX-2]` APPLIED 2026-08-03 — the retired `Request Inbound` wording is gone from the drawing.)* `[L-F2]` §3.18 |
...
| **QA-EXP-03** | [ADMIN] | **Given** audit mode is active, **When** `⬇ Export` is clicked, **Then** the two audit columns `Counted Qty` and `Diff` are included, **no money column appears in the file**, the session id appears in the filename, and `[DC-19]` records `audit-mode = true`. `[E-54]` `[BR-31]` |
...
| **QA-GLB-08** | [WF] | **NEGATIVE.** **Given** every pane, **Then** all three clauses hold. **(1)** No live scan-feed list: zero elements whose `id` or `class` matches `/scan / feed/i`. **(2)** No scan counter: zero occurrences of `/scan/i` in the normalised `innerText` of any `.pane` or `.overlay`. **(3)** No self-refocusing input. A scan loop is defined here as *focus returning to a barcode-class input without a user gesture*; a plain input merely **retaining** focus after `Enter` is not a scan loop and is not what this test forbids — so the two probes below assert on **return**, never on retention. Let `el` be the input the agent typed into, and call any element *scan-class* when `element.matches('.bcin, #auSearch')` returns `true`. **Probe 1 — no focus hop.** Focus the `.bcin` of SKU `100024743`, type `8809738317481`, press `Enter` — **then** assert that `document.activeElement` is either `el` itself **or** an element for which `document.activeElement.matches('.bcin, #auSearch')` returns `false`. Focus may legitimately stay on `el`; it must not jump to a *different* scan-class field. **Probe 2 — no focus return.** From that state vacate focus once — `document.activeElement.blur()`, or a click on the non-focusable `.ptitle h2` — then, with no further input, wait `400 ms` or longer and assert `document.activeElement.matches('.bcin, #auSearch') === false`. Nothing may pull focus back on its own. Repeat both probes with `#auSearch` in audit mode (type `zzzz`, press `Enter`) and assert the same two things. Baseline: the shipped file registers no timer, interval, or blur handler that re-focuses either input; its single `focus()` call sits inside the `#auAdd` **click** handler — a user gesture, asserted by `QA-AUD-14` — and is therefore outside what this clause forbids. `[G-1]` is scoped to View Orders and Closing; the `.bcin` and search inputs accept keyboard-wedge input but are not a scan loop. §9.1 #11 |
| **QA-GLB-09** | [WF] | **NEGATIVE.** **Given** the whole page including all six modals, **Then** every Hangul run in the normalised `innerText` is a substring of one of the **16 allow-listed strings enumerated in §8.0** (11 Current Stocks `Product Name KR` values + 5 `[L-13]` catalogue names), and nothing else. Any Hangul appearing inside a `th`, `label`, `button`, `h2`, `h4`, `.paneheader`, or `.form-note` is Korean UI chrome and fails this test outright. Baseline: the shipped wireframe now contains **no non-product Hangul at all**. *(17 → 16 on 2026-08-04: the seventeenth entry, the data string `신규`, existed only as `— (신규)` inside the retired `Loss` cells; both cells went with the column, so the contract's Hangul is now exactly the product names.)* `[G-6]` `[BR-4]` |
...
| **QA-GLB-11** | [WF] **rewritten 2026-08-04** | **NEGATIVE.** **Given** the normalised `innerText` of every pane and all six modals, **Then** **no forbidden money token appears anywhere** — Inventory renders no monetary value and no currency mark at all `[BR-31]` `[E-72]`. **The forbidden set is canonical for this page and every other row cites it rather than restating it**; matching is case-insensitive: `₩` · `KRW` as a whole word · `$` · `USD` as a whole word · the phrase `product cost` · the column-header fragment `Loss (` · and a bare `원` at a token boundary (`/원(?![^\s])/`). Signed **counts** (`−1`, `+2`, `ADJUST(−3)`) carry no currency mark and are not in scope. Baseline: the shipped wireframe contains **zero** occurrences of `₩`. *(Until 2026-08-04 this row was a KRW **formatting** check with the census `+₩15,000` · `+₩46,260` · `+₩61,260` · `₩0` · `−₩15,000` · `−₩17,800` · `−₩28,400` · `−₩36,000` · `−₩61,260` · `−₩128,460` plus `— (신규)`; money was removed from the audit, so the same ID now asserts absence instead of format.)* `[BR-31]` `[BR-4]` `[E-72]` |
...
| DC-8 | QA-AUD-25 *(QA-AUD-32 / QA-AUD-33 retired 2026-08-04 — both asserted the removed cost fields, and neither carried a unique DC-8 clause; QA-AUD-25 already asserts the event's persistence)* | DC-25 | QA-AUD-24 (present), QA-AUD-43 (absent) |
...
| 8 | **Procurement Hub links, tabs, or embedded views** — and, since 2026-08-04, **the costing read too**. | 2026-08-02 (owner: excluded from this program entirely) · costing read died 2026-08-04 | Separate future workstream. Its last touchpoint was a FIFO lot-cost **read** feeding `[BR-4]`'s loss column; with money removed from the audit, Inventory has no dependency on Procurement Hub of any kind | QA-NAV-08 |
...
| 16 | **Any money on the Stock Audit — and any money anywhere on Inventory.** Specifically: the `Loss (₩)` column on Current Stocks, the `Loss` column in `[L-M1]` and `[L-M2]` / `[L-M2b]`, the `Total Loss` column in `[L-15]`, a cost-weighted total or monetary target in `#auditSummary` or any note or toast, a product-cost lookup of any kind, and a money field on `[DC-6]`, `[DC-8]`, or `[DC-10]`. | **2026-08-04 (owner)** | The audit reports **unit** differences — *"그냥 −1 +1 이런 것만 보이게 하자"*. Money on top of the count was a derivation whose cost basis was never decided (`OQ-1`, now void), and it made a floor tool carry a finance figure. The counting behavior is unchanged; only the valuation is gone `[BR-4]` `[BR-31]`. **Scope boundary — these are NOT covered and must stay:** the Inbound Request form's `Unit Cost` / `JIT Price` (purchase records, owned by `inbound-request`) and Order Detail's `Subtotal` / `Total Discount` / `Total Amount` (WooCommerce order data). Neither is rendered on Inventory | QA-GLB-11, QA-CS-14, QA-AUD-06, QA-LOG-02 |
...
> **`OQ-1` — VOID 2026-08-04, removed from this table.** It asked *"Where is the audit `Loss (₩)` product cost sourced from?"* (FIFO lot cost vs latest purchase price, never decided). The owner removed money from the Stock Audit entirely, so **the question has no subject: there is no loss figure to cost and no cost to source** `[BR-4]` §9.1 #16. It is not deferred, not reassigned, and not answered — it is invalidated, and nothing downstream is blocked by it any more. The ID is retained here as a tombstone so a reader of an older revision does not go looking for it in the table below, and so nobody re-opens the costing question as a prerequisite for shipping the audit. **This section now carries exactly one NO-DEFAULT item: `OQ-2`.**

| ID | Question | Owner | Blocking |
|---|---|---|---|
| **OQ-2** — **RESOLVED, OWNER-DECIDED 2026-08-04** | **When one SKU has been inbounded through more than one sourcing route, which single value does its `Sourcing Route` cell show?** **Answer: the route of the FIFO *active receipt* — the inbound receipt the units currently on hand belong to.** Receipts are consumed oldest-first, so the on-hand quantity is attributed by walking receipts backwards from the most recent until that quantity is covered; the cell shows the route the **next unit out** would come from. The same FIFO attribution the COGS calculation uses to stamp a PO number — **the logic only, with no Procurement Hub read** ([G-5], amended 2026-08-04). The value is derived and changes as stock moves, which is correct for a current-stock cell; it is **snapshotted, never recomputed, when it attaches to an order line**; and stock with **no** inbound receipt (audit increases, `[BR-7]` JIT residual) shows the **unknown marker** rather than inheriting a neighbour's route.  Original framing, kept so the question the answer resolves stays legible: §3.1 fixes exactly one route cell per SKU row and `[E-67]` treats a repeated SKU row as an upstream data fault, so a SKU × route row set is already excluded — but nothing in this document, in `[G-5]`, or in `[BR-7]` / `[BR-8]` / `[E-53]` states how the one surviving value is resolved. **The premise is corpus-confirmed, not hypothetical:** `inbound-request.md` §3.1.2 states *"Route is request-level, not row-level"*, and one request carries many SKU lines, so two requests for one SKU carry two routes; separately `[BR-7]` JIT residual stock arrives **order-side, through no inbound request at all**, on a SKU that may also be replenished as `SMART BUY`. The candidate answers are observably different — a value fixed on the product master, the **first** inbound's route, the **latest** inbound's route, or a per-lot route with some display rule — and this document defines **no `route.*` event and no route field on `[DC-1]`**, so a change of the displayed value would today be neither specified nor traceable. **Not decided anywhere; no provisional default exists** — `[PD-80]` governs only how `OTHER` *renders*, never how a route is *resolved*. | Owner (which value an operator must see), then development (the resulting data model) | Blocks the `Sourcing Route` cell of `[L-5]`, the route filter's result set `[E-53]`, the `Sourcing Route` row of the `🧴 Product Information` card `[L-F1]`, and the route column of both exports `[L-F4]` `[DC-19]`. **Cross-page:** View Orders' existing-inventory exception renders *"that stock's own sourcing route"* (`view-orders.md` `[L-S1-6]`), so the same answer decides what an order line shows. **This is a schema decision** — product-master attribute vs per-lot attribute — and therefore cannot be deferred past data-model design without a re-model |

*(No further NO-DEFAULT item belongs to this page beyond OQ-2 — OQ-1 was voided 2026-08-04, see the note above. `[PD-51]`, `[PD-55]`, `[PD-66]`, `[PD-71]`, `[PD-74]`, and `[PD-79]` are NO-DEFAULT items owned by other screens and are named here only so a reader does not go looking for them.)*
...
`[PD-1]` role model (`[BR-28]`) · `[PD-2]` send sound on `－ Record Outbound` (§6.5) · `[PD-3]` append-only comments (`[BR-23]`) · `[PD-4]` Slack failure handling (`[BR-30]`) · `[PD-5]` destructive-action confirms (§3.3 exit, §3.16) · `[PD-6]` stale-entity revalidation (§3 preamble) · `[PD-7]` concurrency (`[E-7]`) · `[PD-8]` tracking namespaces (`[E-63]`) · `[PD-9]` no automatic carrier capture (`[BR-33]`) · `[PD-13]` location-before-inbound boundary (`[E-43]`) · `[PD-22]` JIT residual origin (`[BR-7]`) · `[PD-40]` counted-qty prefill (`[BR-12]`) · `[PD-41]` PENDING display-only (`[BR-18]`) · `[PD-42]` Barcode search key (`[L-F1]`) · ~~`[PD-43]` cost visibility on the floor (`[BR-29]`)~~ — **moot 2026-08-04: there is no cost surface to make visible or hide** `[BR-4]`; the register entry needs no answer from the owner and reversing it would not restore anything · `[PD-44]` single audit session (`[BR-14]`) · `[PD-45]` release on an active order (`[BR-21]`) · `[PD-46]` location 1:1 exclusivity (`[BR-1]`) · `[PD-47]` filtered audits and scope (`[BR-15]`, `[BR-34]`, **`[BR-35]`**, `[E-11]`, `[E-81]`, §3.11 — note that `[BR-35]` **extends** the register's wording, which counts scope rows only; the owner must be asked for the extension, and reversing it changes `SKUs Checked` from scope+additions to scope-only) · `[PD-48]` shortage acknowledgement (`[BR-13]`) · `[PD-49]` under-restock remainder (`[BR-20]`) · `[PD-50]` over-restock cap (`[BR-20]`) · `[PD-63]` photo removal (§9.1 #7) · `[PD-67]` pool-entity comment routing (`[E-56]`) · `[PD-80]` `OTHER` route rendering (`[BR-8]`).

**Developer decisions (no owner input needed).** Each states a default and is dev-owned.

| Area | Decision |
|---|---|
| ~~Costing~~ | **REMOVED 2026-08-04.** There is no costing work on this page: no basis to choose, no lookup to build, no missing-cost branch to handle `[BR-4]` §9.1 #16. A developer who finds a cost lookup wired into any audit path should delete it, not configure it |
...
Every decision that shaped this screen, 2026-07-09 → 2026-08-04, including reversals and removals. Nothing is silently dropped — the 2026-07-22 rows that introduced the loss column and its cost deferral are **kept as history** and are superseded by, not replaced with, the 2026-08-04 rows at the end.
...
| 2026-07-22 | **`Loss = Diff × product cost` fixed as the design; the total targets 0** as the centre manager's KPI. **The cost source is explicitly deferred to development** (FIFO lot cost recommended vs latest purchase price). — **SUPERSEDED 2026-08-04: reversed in full.** The design is retired and the deferred cost question is void; kept here as the history of why the loss column ever existed. | Rule + deferral | Plan ledger item E → `[BR-4]`, §9.2 OQ-1 (both now retired/void) |
...
| 2026-08-03 | **The audit summary bar is visible only in audit mode** (`#auditSummary` gains class `audcol`), joining the audit-only columns and the unregistered-product row. *(Those columns were three on this date and are two since 2026-08-04 — `[BR-5]`. The visibility rule itself is unchanged.)* | Rule | commit `d09fe79` → `[BR-5]` `[G-14]` |
...
| 2026-08-03 | **Comment `@mention` channel CONFIRMED: `#fulfillment-admin-comments` (`C0BMGEWM5QA`)**, message body @mentions the person. Supersedes the "pending" wording in the earlier global-rules draft. | Global rule | `_inputs/slack-routing.md`; review conflict C-2 → §6.1 |
...
| 2026-08-03 | **`Loss (₩)` and product cost stay visible to whoever runs the audit** — no admin-only split in v1. — **SUPERSEDED 2026-08-04: moot, not reversed.** Money was removed from the audit outright, so there is no cost surface whose visibility could be split. | Rule (provisional) | `[PD-43 · OWNER-PENDING]` → `[BR-29 · retired]` |
...
| 2026-08-03 | **Money on this page is KRW (`₩`)** and timestamps are **KST**, `YYYY-MM-DD HH:MM` (seconds in session detail), with no relative times. Declared explicitly so the org-wide USD reporting convention is not applied to this operational ledger. — **Currency half SUPERSEDED 2026-08-04**: there is no money on this page at all, so `[BR-31]` / `[E-72]` became absence rules. **The timestamp half (`[BR-32]`, `[E-71]`) is untouched and still binding.** | Rule | `[BR-31]` `[BR-32]`, `[E-71]` `[E-72]` |
...
| **2026-08-04** | **REVERSAL — money is removed from the Stock Audit, and from Inventory entirely.** Owner: *"재고 실사의 손실액은 계산하지 말자. 그냥 −1 +1 이런 것만 보이게 하자. 금액은 다 빼서 모든 곳에 반영하자."* The audit reports **unit differences only**. Deleted: the `Loss (₩)` column on Current Stocks (audit columns 3 → 2), the `Loss` column in `[L-M1]` (8 → 7) and `[L-M2]` / `[L-M2b]` (7 → 6), the `Total Loss` column in `[L-15]` (7 → 6), the money `#auditSummary` string, the money clauses in the `[L-M1]` / `[L-M2]` / `[L-M2b]` / `[L-15]` notes and in the confirm toast, every product-cost lookup, and the money fields on `[DC-6]`, `[DC-8]`, `[DC-10]`. Replaced with counts already in the document's vocabulary: `Adjustments: {n} ({−a} / {+b})` and a separate `New additions: {m}`. The centre manager's KPI is now the **adjustment count** toward 0. | **Reversal / removal** | Owner decision 2026-08-04 → `[BR-4]`, `[BR-5]`, `[BR-28]`, `[BR-29 · retired]`, `[BR-31]`, §3.1, §3.3, §3.9, §3.11, §3.13, §3.14, §5.1, §6.3, §9.1 #16 |
| **2026-08-04** | **Consequent retirements, IDs kept, never renumbered.** Rules: `[BR-4]` money design retired and replaced in place · `[BR-29]` retired (nothing left whose visibility could be role-split) · `[BR-31]` converted from a KRW **formatting** rule into the negative *"no money renders anywhere on this page"*. Edge cases: `[E-25]` (no product cost) and `[E-61]` (cost changes mid-session) retired as unreachable; `[E-72]` converted to the money-absence negative. Open questions: **§9.2 `OQ-1` VOID** — the costing-basis question has no subject and is not deferred, reassigned, or answered; §9.2's `Costing` developer-decision row removed; `[PD-43]` marked moot in the traceability index. Integration: §6.3's Procurement Hub **read dependency dropped** — Inventory now depends on it for nothing (§9.1 #8). | Removal / hygiene | Owner decision 2026-08-04 |
| **2026-08-04** | **Wireframe and spec moved together — no divergence declared, no `[INV-WFX]` entry owed.** `wms2/stock-status/index.html` dropped every money surface in the same round: the third `th.audcol`/`td.audcol` `Loss (₩)`, the `#auditSummary` money string, `[L-15]`'s `Total Loss` column and its "bring the total loss to 0" note, the `Loss` columns and money notes in `#m-adjust` / `#m-adjlog` / `#m-adjlog6`, and both `— (신규)` cells. The drawing carries an inline "do not re-add" comment at each removal site, so the constraint survives in the SST as well as here. **Consequences for §8:** 2 scenarios RETIRED (`QA-AUD-32`, `QA-AUD-33` — product-cost preconditions), 19 rewritten in place (13 [WF] + 6 [ADMIN]), **0 blocked; all 198 live scenarios are runnable.** `QA-LOG-09` was **rewritten, not retired** — the `Total Loss` colour scale died with the column, but §3.11's surviving green-zero rule moved onto the `Adjustments` column under the same ID, asserted there and nowhere else. The prehandoff runner stays at 75 executed scenarios and passes 75/75. Removing the two `— (신규)` cells also removed the page's last non-product Hangul, taking the `QA-GLB-09` allow-list from **17 strings to 16**. Precedent for handling it this way rather than as a declared divergence: the 2026-08-03 Comments-hub string resolution (§3.12), where wireframe, spec and QA moved in one commit. | Wireframe + spec | Owner decision 2026-08-04; deploy through `/wf-deploy stock-status` |

---

*End of specification. 21 implementation units (16 legend + 5 page furniture) · 36 business rule IDs (35 live, `[BR-29]` retired 2026-08-04; `[BR-4]` and `[BR-31]` replaced in place) · 34 data-capture events + 13 declared non-events · 98 edge-case IDs (96 live, `[E-25]` and `[E-61]` retired 2026-08-04; `[E-72]` replaced in place) · 200 QA scenario IDs (198 live — 75 [WF] / 123 [ADMIN], all runnable; 2 retired 2026-08-04; 85 negative/boundary = 42.5%) · 25 provisional decisions applied (`[PD-43]` moot since 2026-08-04) · **1 NO-DEFAULT open question (`OQ-2`) — `OQ-1` voided 2026-08-04** · 2 wireframe defects raised (`[INV-WFX-1 · proposed]`, `[INV-WFX-2 · proposed]`) — the 2026-08-04 money removal needed none, wireframe and spec having moved together. **No ID was renumbered or reused.***
