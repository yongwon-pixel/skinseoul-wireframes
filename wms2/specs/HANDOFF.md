...
>
> **Status:** specs frozen for handoff, 2026-08-03, with **three owner amendment rounds applied since**. Round 1 (2026-08-04): loss-amount removed from the stock audit, now counts-only (`[G-14]`). Round 2 (2026-08-04): the last four NO-DEFAULT questions answered — sourcing route resolves to the FIFO active receipt and an order is JIT when any line is (`[G-5]`), marketing-import orders carry amount `0`, scan-feed export stays open but logged, and new **`[G-17]`** stops an order ever being created without a shippable carrier (which reverses `order-management.md` `BR-20`, voids `[PD-55]` and `[RTO-X-1]`, and retires the `carrier_unresolved` state). **No NO-DEFAULT question remains open.** Rounds 1 and 2 left the scenario census unchanged at 647 `[WF]`, all passing. Round 3 (2026-08-08): **manual stock adjustment** — a person can restate a SKU's `Total` from the Inventory row, and every `ADJUST` row now states which of its three writers produced it (`stock-status.md` `[L-19]` / `[L-20]`, `_global-rules.md` v1.6). It is the only round that moved the census: `stock-status` gained 29 scenario IDs, **15 of them `[WF]`**, so the runnable set is now **662 `[WF]`** = 647 + 15 (see §4; the other 14 are `[ADMIN]` and no runner executes them). One wireframe change is still owed and registered as `WF-16b` (see §2.4 of `order-management.md`).
...
1. **`_global-rules.md`** — 17 cross-cutting rules `[G-1]`…`[G-17]` that every screen inherits, plus the canonical cross-page event names, the Slack routing table, and the byte-exact Comments-hub copy. Read it completely before any screen spec. Screen specs cite these rules by ID and state **page deltas only** — they never restate a rule body, so a screen spec read alone will look incomplete.
...
|---|---|---|
| `[G-n]` | **Global rule.** `[G-1]`…`[G-17]`, cross-cutting, inherited by every screen. | `_global-rules.md` |
...
| `OQ-n` | **Open question with no default** — the owner must decide; no behavior is specified anywhere. **As of 2026-08-04 none remain open.** The last four were answered that day: `stock-status.md` **OQ-2** and `ready-to-outbound.md` **OQ-3** (one axis — the sourcing route resolves to the FIFO active receipt, and an order is JIT when any line is; `[G-5]`), `order-management.md` **OQ-1** (marketing orders carry amount `0`, and are excluded from sales by order type, never by `amount = 0`), and `view-orders.md` **OQ-2** (scan-feed export allowed and logged; `delivery = email` restricted to internal domains). `inbound-request.md` **OQ-3** was never open — it carries a specified default (bulk bar only). Every `OQ-n` heading you meet in a spec now records an answer, not a question. `stock-status.md` OQ-1 (the audit loss-amount cost source) was **retired 2026-08-04** when the owner removed loss-amount from the stock audit — see §6. | §9 |
...
_verify/prehandoff/qa-stock-status.py        90
...
Run one with `python3 qa-<screen>.py [--json out.json]`. **The current full-set baseline is 662/662 PASS, measured 2026-08-09 on a clean tree at an unrelated path — no `.git`, no inherited `__pycache__` — with zero failures, zero errors and zero bytes on stderr from any of the eight.** The per-runner numbers in the table above are that run. It supersedes the 647/647 baseline of 2026-08-03: `qa-stock-status.py` grew 75 → 90 with the 2026-08-08 manual stock adjustment (`stock-status.md` §8.0), and 647 + 15 = 662. Any `[WF]` failure you see is therefore a genuine regression, either in the wireframe or in your reading of the protocol.

> **One assertion is load-sensitive, so read a lone failure there before believing it.** `qa-inbound-request.py` `QA-A-19` asserts a toast is *still visible 2.4 s after a second click* — a wall-clock deadline. Running all eight back-to-back on one machine failed it once and passed it on the immediately following three solo runs (57/57 each), so under contention it can report a false failure. It is the only assertion in the set with this property. Re-run that runner alone before treating `QA-A-19` as a regression; every other ID in the 662 is deterministic. Use these as a change-detector when you edit a wireframe: a copy edit that touches an asserted string must be applied to the drawing, the spec, and the runner in the same pass, or the runner will correctly fail.
...
**No `OQ-n` item is open. `stock-status.md` OQ-2 was answered — this paragraph used to say otherwise and was wrong.** Until 2026-08-09 it read *"One item is genuinely open with no default anywhere: `stock-status.md` OQ-2"*, which contradicted §3's own glossary row above (*"As of 2026-08-04 none remain open… the last four were answered that day"*) and `stock-status.md` §9.6, whose table heading reads **RESOLVED, OWNER-DECIDED 2026-08-04**. The stale half is corrected here rather than deleted, because a developer who read the old sentence escalated a settled schema decision.

**The answer, so it is not looked up twice:** when a SKU has been inbounded through more than one sourcing route, its `Sourcing Route` cell shows **the route of the FIFO *active receipt*** — receipts are consumed oldest-first, so the on-hand quantity is attributed by walking receipts backwards from the most recent until it is covered, and the cell shows the route the **next unit out** would come from. The same FIFO attribution COGS uses to stamp a PO number, **the logic only, with no Procurement Hub read** (`[G-5]`, amended 2026-08-04). The value is derived and moves as stock moves, which is correct for a current-stock cell; it is **snapshotted, never recomputed, once it attaches to an order line**; and stock with no inbound receipt (audit increases, manual increases `[BR-43]`, `[BR-7]` JIT residual) shows the **unknown marker** rather than inheriting a neighbour's route. It governs the `Sourcing Route` cell of `[L-5]`, the route filter's result set, the Product Information card's route row, the route column of both exports, and View Orders' existing-inventory exception. **What remains is development work, not an owner question:** the data model still has to carry it — this spec defines no `route.*` event and no route field on `[DC-1]`, so implementing the answer is a schema task that cannot be deferred past data-model design.
