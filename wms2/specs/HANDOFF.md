...
>
> **Status:** specs frozen for handoff, 2026-08-03, with **two owner amendment rounds applied 2026-08-04**. Round 1: loss-amount removed from the stock audit, now counts-only (`[G-14]`). Round 2: the last four NO-DEFAULT questions answered — sourcing route resolves to the FIFO active receipt and an order is JIT when any line is (`[G-5]`), marketing-import orders carry amount `0`, scan-feed export stays open but logged, and new **`[G-17]`** stops an order ever being created without a shippable carrier (which reverses `order-management.md` `BR-20`, voids `[PD-55]` and `[RTO-X-1]`, and retires the `carrier_unresolved` state). **No NO-DEFAULT question remains open.** The scenario census is unchanged at 647 `[WF]`, all passing; one wireframe change is owed and registered as `WF-16b` (see §2.4 of `order-management.md`).
