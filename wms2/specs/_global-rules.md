...
- **What counts as a confirming action — PUBLISHED 2026-08-04 (`CP-9`).** **State-changing actions always confirm.** Pure view-state changes — expand/collapse, tab switch, checkbox tick, opening a modal, `Cancel` — are **not** confirming actions and must not toast. Where a page gives **in-place** feedback for a state change (View Orders' `✓ Saved` chip, `[BR-51]`), that chip **is** the confirmation: it is a delta in *position*, not an absence of one, and it satisfies this rule. **Failures always use the red top-right toast regardless of where the success feedback sits.** Both `view-orders.md` and `ready-to-outbound.md` now cite this instead of each defining the boundary.
...
- **Which surfaces may render `OTHER (channel)` on order-facing rows — PUBLISHED 2026-08-04 (`CP-3`).** **View Orders order rows do; Order Detail does not.** On View Orders it is a declared delta on the "exactly 4" badge set (`view-orders.md` `[L-S1-6]`, `[BR-5]`): a rendering pass-through of the route the line inherited from its Inbound Request, never a fifth selectable route. Order Detail states it never renders OTHER and `order-detail.md` `[X-5]` records that this rule backs Order Detail; that stands. `ready-to-outbound.md` §1.4/§4.2 previously named Order Detail as a rendering surface — **that was the error**, corrected in the same commit. Inventory renders the stock-level route per the FIFO rule above. `[PD-80]` still governs the *label form*, not which surfaces show it.
...
- **Path form — PUBLISHED 2026-08-04 (`CP-8`).** The corpus writes **one** form: `../{slug}/#anchor`. The wireframes ship `../{slug}/index.html#anchor` (measured: exactly one such `href` in the set, `../inbound-request/index.html#reqlist`) because they are a **static file build**, and `[WF]` QA asserts that shipped literal verbatim. This is a declared build artifact, not a second contract: the production admin routes to the entity and never to a file, as the paragraph above already states. No spec should describe the `index.html` form as a requirement, and no `[WF]` scenario should be rewritten to the directory form while the mock is a file build.
